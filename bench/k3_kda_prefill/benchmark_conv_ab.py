...
for n in [1024, 2048, 4096, 8192, 16384]:
    ...
    weights = [
        torch.zeros(D, WIDTH, device="cuda", dtype=torch.float32) for _ in range(3)
    ]
    ...
    states = [
        torch.zeros(1, D, WIDTH - 1, device="cuda", dtype=torch.bfloat16)
        for _ in range(3)
    ]

    def old():
        padded = torch.cat((torch.cat(states, dim=1), current_x), dim=-1)
        return F.silu(F.conv1d(padded.float(), current_w, groups=3 * D)).to(
            torch.bfloat16
        )
    ...
    def new():
        return [
            causal_conv1d_fn(
                ...
                None,
                conv_states=st,
                query_start_loc=qstart,
                cache_indices=cache_idx,
                has_initial_state=has_state,
                activation="silu",
                seq_lens_cpu=[n],
            )
            for x, w, st in zip(parts, weights, states)
        ]
    ...
    row = {
        "chunk": n,
        "current_padded_conv_ms": old_ms,
        "ragged_triton_ms": new_ms,
        "speedup": old_ms / new_ms,
    }
    rows.append(row)
    print(row, flush=True)
out = Path(__file__).parent / "results/conv_ab_b300.csv"
