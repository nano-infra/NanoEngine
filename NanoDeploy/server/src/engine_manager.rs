...
impl EngineManager {
            ...
            for engine in &self.prefill_engines {
                ...
                for target_uuid in &decode_uuids {
                    if let Some(target_resp_bytes) = node_responses.get(target_uuid) {
                        ...
                        if let Ok(p2p_resp) = flatbuffers::root::<P2PInitResponse>(target_resp_bytes) {
                                ...
                                for i in 0..peers.len() {
                                    ...
                                    if peer.id() == Some(my_uuid.as_str()) {
                                        if let Some(local_info) = peer.local_info() {
                                            target_map.insert(target_uuid.clone(), local_info.to_vec());
                                        }
                                        break;
                                    }
                                }
    ...
    pub async fn start_etcd_watch(manager: Arc<Mutex<EngineManager>>, config: EtcdConfig) {
        ...
        tokio::spawn(async move {
            ...
            // 1. Initial Scan
            match client.get(prefix.clone(), Some(GetOptions::new().with_prefix())).await {
                Ok(resp) => {
                    let kvs = resp.kvs();
            ...
            // 2. Watch Loop
            match client.watch(prefix, Some(WatchOptions::new().with_prefix())).await {
                                    ...
                                    match event.event_type() {
                                        EventType::Put => {
                                            if let Some(kv) = event.kv() {
                                                EngineManager::handle_node_update(&manager_clone, kv.value()).await;
                                            }
                                        }
                                        EventType::Delete => {
                                            if let Some(kv) = event.kv() {
    ...
    async fn handle_node_update(manager: &Arc<Mutex<EngineManager>>, val: &[u8]) {
        ...
        if !mgr.known_nodes.contains_key(uuid) {
             info!("Discovered new node: {} ({}) at {} [status={}]", uuid, role, addr, status);
             ...
             mgr.known_nodes.insert(uuid.to_string(), Arc::new(Mutex::new(adapter)));
        } else {
        ...
        if status == "ready" {
            if role == "prefill" {
                ...
                let exists = mgr.decode_engines.iter().any(|a| Arc::ptr_eq(a, &adapter_arc));
                if !exists {
            ...
            // Remove from pool if present (e.g. went back to initializing or unhealthy)
             if role == "prefill" {
                 mgr.prefill_engines.retain(|a| !Arc::ptr_eq(a, &adapter_arc));
             } else if role == "decode" {
