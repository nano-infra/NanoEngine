...
async fn test_mock_engine_ipc() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let mut head = [0u8; 12];
        socket.read_exact(&mut head).await.expect("Failed to read header");
        ...
        socket.read_exact(&mut meta).await.expect("Failed to read meta");

        let data_size = u32::from_le_bytes(head[8..12].try_into().unwrap());
        if data_size > 0 {
            let mut data = vec![0u8; data_size as usize];
            socket.read_exact(&mut data).await.expect("Failed to read data");
            assert!(data.len() > 10);
        }
    });
    ...
    client.connect(&addr.to_string()).await.expect("Connect failed");

    // Send an application-specific request (Add Sequence)
    let tokens = vec![1, 2, 3];
    client.send_add_request(999, &tokens).await.expect("Send failed");
