...
template<typename T>
SGL_DEVICE void c4_write(T*            kv_score_buf,  //
...
SGL_DEVICE void c4_forward(const InFloat*                  kv_score_buf,
...
C4_KERNEL void flash_c4_decode(const __grid_constant__ Compress4DecodeParams params)
{
    ...
    const auto& [_kv_score_buffer,
                 _kv_score_input,
                 _kv_compressed_output,
                 _score_bias,  // kv score
                 indices,
                 seq_lens,
                 extra,
                 batch_size  // decode info
    ]                         = params;
...
struct FlashCompress4Kernel {
    ...
    static void run_decode(const tvm::ffi::TensorView                     kv_score_buffer,
    ...
    static void run_prefill(const tvm::ffi::TensorView                     kv_score_buffer,
                            const tvm::ffi::TensorView                     kv_score_input,
                            const tvm::ffi::TensorView                     kv_compressed_output,
                            const tvm::ffi::TensorView                     ape,
                            const tvm::ffi::TensorView                     indices,
...
private:
    static const void* _get_extra_pointer(host::SymbolicSize&                             B,  // batch_size
