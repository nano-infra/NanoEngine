...
struct RegisterTopK {
    ...
    SGL_DEVICE static void run(const float*   scores,  //
                               int32_t*       indices,
...
#pragma unroll
        ...
        if constexpr (kIs2Pass) {
            ...
            if (lane_id == 0)
                ptx::mbarrier_wait(&smem->mbarrier, 0);
            __syncwarp();
