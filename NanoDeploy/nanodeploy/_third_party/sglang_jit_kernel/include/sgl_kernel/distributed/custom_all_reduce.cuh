...
public:
    ...
    CustomAllReduceBase(uint32_t rank,
                        uint32_t num_gpu,
    ...
    tvm::ffi::Array<InputPair> share_graph_inputs()
    {
        ...
        for (const auto ptr : std::span(m_graph_capture_inputs).subspan(m_cum_registered_count)) {
            ...
            void*      base_ptr;
            const auto cu_result =
                cuPointerGetAttribute(&base_ptr, CU_POINTER_ATTRIBUTE_RANGE_START_ADDR, (CUdeviceptr)ptr);
    ...
    void register_inputs(tvm::ffi::Array<tvm::ffi::Array<InputPair>> ipc_graph_inputs)
    {
        ...
        if (new_registered_count == 0)
            return;  // avoid `m_get_data_ptr()` out-of-bounds
