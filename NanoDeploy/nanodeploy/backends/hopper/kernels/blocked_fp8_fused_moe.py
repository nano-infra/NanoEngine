...
def fused_moe_blocked_f8_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        a_scale = tl.load(
            as_ptrs + offs_ksa * stride_ask, mask=mask_sid and k_start < K, other=1.0
        )
...
def _make_intermediate(
    shape: tuple, dtype: torch.dtype, device: torch.device, zeros: bool
):
    """make intermediate."""
    if zeros:
...
def dlblas_fused_moe_blocked_fp8(
    input: torch.Tensor,
    ...
    intermediate_cache1 = _make_intermediate(
        (M, topk, N), dtype=out_dtype, device=device, zeros=not full_exp
    )
    # gate and up
    ...
    intermediate_cache2 = _make_intermediate(
        (M, topk, w2.shape[1]), dtype=out_dtype, device=device, zeros=not full_exp
    )
    # down
    fused_moe_blocked_fp8_kernel_launcher(
        gate_cache,
