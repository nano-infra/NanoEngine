...
from nanodeploy.fbs.P2PInitResponse import P2PInitResponse, P2PInitResponseT
    ...
    def _send_response(self, action: int, payload: bytes, seq_id: int = 0):
        if self.active_queue:
            packet = engine_rpc_pb2.StreamPacket(
                seq_id=seq_id, action=action, payload=payload
            )
...
class EngineServer:
    ...
    async def serve(self):
        ...
        await asyncio.gather(server.wait_for_termination(), self.service.engine_loop())
