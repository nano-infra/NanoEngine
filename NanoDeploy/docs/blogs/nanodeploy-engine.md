...
主流推理框架（如 vLLM）经过长期的开源演进，功能虽全，但代码库已相对庞大，开发者若想深入底层修改通信逻辑或调度策略，往往面临较高的学习门槛。
相比之下，NanoDeploy 坚持精简的底层架构。我们剥离了复杂的历史包袱，高度聚焦于最核心的混合并行与调度机制，使得核心引擎的代码量大幅低于 vLLM。这种高内聚的模块化结构，不仅降低了源码阅读和二次开发的门槛，也让触达底层的极致性能调优变得更为直接。
...
在此基础上，NanoDeploy 进一步集成了 MTP 投机解码（详见 [NanoDeploy - MTP 投机解码](nanodeploy-mtp.md)），通过 Lazy Verify 策略在不增加显著工程复杂度的前提下，利用模型自带的 MTP head 实现了每步多 token 产出，进一步压缩了 decode 延迟。精简的内核架构使得这类前沿优化能够快速落地，而不必在庞大的历史代码库中艰难穿行。
