...
int main(int argc, char **argv) {
  ...
  std::cout << "[Orchestrator] Connecting to Hub at " << hub_ip << ":"
            << hub_port << "..." << std::endl;
  ...
  std::cout << "\n[Orchestrator] Starting Pipeline: (" << initial_value
            << " + 10)^2" << std::endl;

  std::cout << "  -> Sending " << initial_value << " to Agent Alpha (AddTen)..."
            << std::endl;
  auto f1 = client_alpha.callRemote<double, double>("agent_alpha", kAddTen,
                                                    initial_value);

  double mid_result = f1.get();
  std::cout << "  <- Alpha returned: " << mid_result << std::endl;

  std::cout << "  -> Sending " << mid_result << " to Agent Beta (Square)..."
            << std::endl;
  auto f2 =
      client_beta.callRemote<double, double>("agent_beta", kSquare, mid_result);
  ...
  auto p1 =
      client_alpha.callRemote<double, double>("agent_alpha", kAddTen, 100.0);
