...
int main(int argc, char **argv) {
  ...
  std::cout << "[Smoke v8] Allocation Success. Ticket: " << alloc_resp.ticket_id
            << std::endl;
  ...
  std::cout << "[Smoke v8] Establishing RDMA channel for " << actor0_id << "..."
            << std::endl;
  if (!client.initRDMA(actor0_id)) {
    std::cerr << "[Smoke v8] Failed to init RDMA for " << actor0_id
              << std::endl;
    return 1;
  }

  std::cout << "[Smoke v8] Establishing RDMA channel for " << actor1_id << "..."
            << std::endl;
  if (!client.initRDMA(actor1_id)) {
    std::cerr << "[Smoke v8] Failed to init RDMA for " << actor1_id
              << std::endl;
    return 1;
  }
  ...
  auto fut0 = client.callRemote<int, int>(actor0_id, (spoke::Action)0x10,
                                          5); // 5^2 = 25
  auto fut1 = client.callRemote<std::string, std::string>(
      actor1_id, (spoke::Action)0x11, "HelloRDMA");

  int res0 = fut0.get();
  std::string res1 = fut1.get();

  std::cout << "[Smoke v8] Result from " << actor0_id << ": " << res0
            << " (Expected 25)" << std::endl;
  std::cout << "[Smoke v8] Result from " << actor1_id << ": " << res1
            << " (Expected Pong: HelloRDMA)" << std::endl;
