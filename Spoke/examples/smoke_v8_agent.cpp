...
public:
  ...
  RdmaMathActor(const std::string &id, int rx, int tx)
      : spoke::Actor(id, rx, tx) {
    std::cout << "[RdmaMathActor] " << id << " started and ready for RDMA."
              << std::endl;
  }

  // SPOKE_METHOD for square operation
  // Explicitly cast the ID to spoke::Action to match the enum class
  SPOKE_METHOD(RdmaMathActor, square, static_cast<spoke::Action>(0x10), int,
               int) {
  ...
  SPOKE_METHOD(RdmaMathActor, ping, static_cast<spoke::Action>(0x11),
