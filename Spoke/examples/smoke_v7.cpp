...
int main(int argc, char **argv) {
  ...
  std::cout << "[Smoke v7] Connecting to Hub for Gang Allocation..."
            << std::endl;
  ...
  std::cout << "[Smoke v7] Allocation Success! Ticket: " << alloc_resp.ticket_id
            << std::endl;
  ...
  for (uint32_t i = 0; i < alloc_resp.num_members; ++i) {
    ...
    // Note: For arguments, we just send empty body for now as V2TestActor
    // doesn't parse them yet
    client.launchActor(alloc_resp.ticket_id, i, "V2TestActor", actor_id, "");
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
  }

  std::cout << "[Smoke v7] All actors launched! Check Agent logs for GPU "
               "isolation output."
            << std::endl;
