...
template <> struct Serializer<ComplexTask> {
  ...
  static size_t size(const ComplexTask &t) { return pack(t).size(); }
...
int main() {
  ...
  std::cout << "[Smoke v6] Calling remote with Complex Object via RDMA..."
            << std::endl;

  auto fut =
      client.callRemote<ComplexTask, std::string>("adv_6", kProcessTask, task);
