...
async fn test_generic_echo() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let mut head = [0u8; 12];
        socket.read_exact(&mut head).await.expect("Header read failed");
        ...
        socket.read_exact(&mut meta).await.expect("Meta read failed");

        let data_size = u32::from_le_bytes(head[8..12].try_into().unwrap());
        let mut data = vec![0u8; data_size as usize];
        socket.read_exact(&mut data).await.expect("Data read failed");
    ...
    // Action 1, Seq 100
    client.send_message(1, 100, &payload).await.expect("Send failed");
