...
- [ ] Define `ResourceSpec` protobuf/struct.
- [ ] Update `nanodeploy_agent` to accept `--gpus <n>` flag (manual resource reporting).
- [ ] Update Hub to store and display agent capacities.

### Phase 2: Simple Scheduler

- [ ] Implement `GangScheduler` class in Hub.
- [ ] Support `strict_locality` flag in Spawn RPC.
- [ ] Basic matching logic: "Find Agent with N free GPUs".

### Phase 3: Hardware Integration

- [ ] Integrate NVML in Agent to auto-detect GPU count/indices.
- [ ] Support GPU_VISIBLE_DEVICES isolation (Agent sets env var for Actor).
