...
- Pipeline parallelism for splitting decoder layers across stages is coming soon; see the feature matrix for status.
...
- GLM multi-step MTP targets Hopper with `attention_tp=attention_sp=1`. Colocated hybrid serving uses `pp=1`; PD serving additionally supports a PP prefill engine paired with a `pp=1` decode engine (`pp=8` prefill and `attention_dp=8, ffn_ep=8` decode is the 1M-context topology). Set `num_speculative_tokens=5` on both PD roles for five recurrent calls to the single checkpoint predictor and a derived six-token target verification span. The predictor runs only on the final prefill PP stage; its KV and first 5-token draft line migrate through the data plane. For long-context PP prefill, keep each stage's microbatch bounded (validated with `--max_num_batched_tokens 16384`) instead of issuing the whole prompt as one GPU forward. On the decode role, add `--enable_hisparse true --hisparse_device_buffer_size 12288`: six DSA top-k rows at `index_topk=2048` share a request-wide hot union while retaining six distinct output KV slots. Tree speculation and PP decode are not enabled.
