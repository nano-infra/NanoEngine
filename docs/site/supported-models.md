...
- GLM multi-step MTP targets Hopper with `attention_tp=attention_sp=pp=1`; both colocated hybrid serving and PD-disaggregated prefill/decode roles are supported, and `attention_dp=8, ffn_ep=8` is the primary topology. Set `num_speculative_tokens=5` on both PD roles for five recurrent calls to the single checkpoint predictor and a derived six-token target verification span. Predictor KV uses normal GPUDirect RDMA migration; a registered per-sequence MTP handoff region carries the first 5-token draft line. Tree speculation, PP, and HiSparse composition are not enabled.
