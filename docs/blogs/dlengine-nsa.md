...
FlashMLA kernel 在处理 FP8 cache 时对内存对齐有要求。DLEngine 采用 **block_size+1 padding** 策略：
...
5. **UE8M0 Graph-Safe 量化**：标准的 `deep_gemm.per_token_cast_to_fp8` 内部调用了 `.item()`，会打断 CUDA Graph 捕获。DLEngine 实现了纯 tensor 操作的替代版本（`_per_token_cast_to_fp8_ue8m0`），通过 `torch.exp2(torch.ceil(torch.log2(...)))` 代替 `.item()` 来计算 UE8M0 scale。
...
FlashMLA 的稀疏 decode kernel 需要 `sparse_tile_scheduler_metadata`。DLEngine 通过 Context 单例管理这个状态，首次调用时创建，后续 graph replay 复用：
...
DLEngine 为每种缓存独立注册 RDMA Memory Region：
