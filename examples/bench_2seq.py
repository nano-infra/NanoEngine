...
def main():
    ...
    # --- Manual Override for Preemption Test ---
    # Variables to control request lengths
    # Total Capacity: ~432,704 tokens
    # Strategy: 
    # 1. R1 (215k) starts. Used: 215k. Free: ~217k.
    # 2. R2 (215k) starts. Used: 430k. Free: ~2k.
    # 3. Both generate ~1000 tokens.
    # 4. Total hits 432k limit -> PREEMPTION triggered.
    tt = 386_304
    hh = tt//2-800
    SEQ1_INPUT_LEN = hh
    SEQ1_OUTPUT_LEN = 1000
    
    SEQ2_INPUT_LEN = hh
    SEQ2_OUTPUT_LEN = 900

    print(f"\n[Preemption Test] Scheduling 2 requests:")
    print(f"  Req 1: In={SEQ1_INPUT_LEN}, Out={SEQ1_OUTPUT_LEN} (Arrival: 0.0s)")
    print(f"  Req 2: In={SEQ2_INPUT_LEN}, Out={SEQ2_OUTPUT_LEN} (Arrival: 10.0s)")

    prompts = [
        [randint(0, 10000) for _ in range(SEQ1_INPUT_LEN)],
        [randint(0, 10000) for _ in range(SEQ2_INPUT_LEN)]
    ]
    sampling_params_list = [
        SamplingParams(temperature=0.6, ignore_eos=True, max_tokens=SEQ1_OUTPUT_LEN),
        SamplingParams(temperature=0.6, ignore_eos=True, max_tokens=SEQ2_OUTPUT_LEN)
    ]
    
    # Send seq1 at t=0, seq2 at t=10s
    arrival_times = [0.0, 5]
    args.num_requests = 2
    # -------------------------------------------
