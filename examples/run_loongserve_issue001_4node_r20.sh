...
MASTER_IP="${MASTER_IP:-10.102.199.30}"
RAY_ADDR="${RAY_ADDR:-${MASTER_IP}:7789}"
...
LOONGSERVE_MIN_COMP_BOUND_BATCH_SIZE="${LOONGSERVE_MIN_COMP_BOUND_BATCH_SIZE:-128}"
...
RUN_TAG="${RUN_TAG:-loongserve_issue001_4node_dp${DP}sp${SP}_r${REQUEST_RATE}_$(date -u +%Y%m%d_%H%M%S)}"
...

{
    ...
    echo "Max Num Recv Seqs: $MAX_NUM_RECV_SEQS"
...
log "RUN_TAG=$RUN_TAG"
log "START loongserve issue001 4node rate=$REQUEST_RATE n_reqs=$NUM_REQUESTS output=$CURRENT_LOG_DIR"
log "RAY_ADDR=$RAY_ADDR MASTER_ADDR=$MASTER_ADDR topology=dp${DP}sp${SP}ep${EP}"
log "max_num_recv_seqs=$MAX_NUM_RECV_SEQS loongserve_min_comp_bound_batch_size=$LOONGSERVE_MIN_COMP_BOUND_BATCH_SIZE profile=${LOONGSERVE_DECODE_PROFILE_PATH:-fallback}"
