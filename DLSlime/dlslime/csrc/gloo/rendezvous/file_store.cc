...
void FileStore::set(const std::string& key, const std::vector<char>& data)
{
...

    {
        ...
        GLOO_ENFORCE(ofs.is_open(), "File cannot be created: ", tmp, " (", ofs.rdstate(), ")");
...
std::vector<char> FileStore::get(const std::string& key)
{
    ...
    if (!ifs) {
        GLOO_ENFORCE(false, "File cannot be opened: ", path, " (", ifs.rdstate(), ")");
    }
...
void FileStore::wait(const std::vector<std::string>& keys, const std::chrono::milliseconds& timeout)
{
    ...
    while (!check(keys)) {
        ...
        if (timeout != kNoTimeout && elapsed > timeout) {
            GLOO_THROW_IO_EXCEPTION(GLOO_ERROR_MSG("Wait timeout for key(s): ", ::gloo::MakeString(keys)));
        }
        /* sleep override */
