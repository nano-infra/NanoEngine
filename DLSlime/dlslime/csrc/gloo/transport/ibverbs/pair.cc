...
Pair::Pair(int rank, const std::shared_ptr<Device>& dev, std::chrono::milliseconds timeout):
    rank_(rank),
    dev_(dev),
    sync_(false),
    busyPoll_(false),
    ...
    ex_(nullptr)
{
    ...
    // Create completion queue
    {
        ...
        cq_ = ibv_create_cq(dev_->context_, kCompletionQueueCapacity, this, dev_->comp_channel_, 0);
    ...
    // Init queue pair
    {
        ...
        rv = ibv_modify_qp(qp_, &attr, IBV_QP_STATE | IBV_QP_PKEY_INDEX | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS);
        GLOO_ENFORCE_EQ(rv, 0);
    }
    ...
    // full address struct in order to connect, and vice versa.
    {
        ...
        GLOO_ENFORCE_EQ(rv, 0);
        rv = ibv_query_gid(dev_->context_, dev_->attr_.port, dev_->attr_.index, &self_.addr_.ibv_gid);
        GLOO_ENFORCE_EQ(rv, 0);
...
void Pair::setSync(bool sync, bool busyPoll)
{
    ...
    if (!busyPoll) {
        GLOO_THROW_INVALID_OPERATION_EXCEPTION("The ibverbs transport only supports busy polling in sync mode");
    }
...
void Pair::sendMemoryRegion(struct ibv_mr* src, int slot)
{
    ...
    GLOO_DEBUG("sendMemoryRegion slot=", slot, " addr=", src->addr, " lkey=", src->lkey);
...
void Pair::recvMemoryRegion(std::unique_lock<std::mutex>& lock, int slot, std::function<void(struct ibv_mr)> callback)
{
    if (sync_) {
        ...
        while (it == peerMemoryRegions_.end()) {
            ...
            pollCompletions();
            lock.lock();
            if (timeout_ != kNoTimeout && (std::chrono::steady_clock::now() - start) >= timeout_) {
                lock.unlock();
                signalIoFailure(GLOO_ERROR_MSG("Timeout waiting for memory region from ", peer_.str()));
...
std::unique_ptr<::gloo::transport::Buffer> Pair::createSendBuffer(int slot, void* ptr, size_t size)
{
...
std::unique_ptr<::gloo::transport::Buffer> Pair::createRecvBuffer(int slot, void* ptr, size_t size)
{
    std::unique_lock<std::mutex> lock(m_);
...
void Pair::send(transport::UnboundBuffer* tbuf, uint64_t slot, size_t offset, size_t nbytes)
{
    ...
    recvMemoryRegion(lock, slot, [this, mr, slot, ptr = buf->ptr, offset, nbytes](struct ibv_mr peer) mutable {
        struct ibv_sge list;
        ...
        GLOO_DEBUG("send UnboundBuffer async slot=",
                   wr.wr_id,
                   " peer_addr=",
                   peer.addr,
                   " remote_addr=",
...
void Pair::recv(transport::UnboundBuffer* tbuf, uint64_t tag, size_t offset, size_t nbytes)
{
    std::unique_lock<std::mutex> lock(m_);
...
{
    GLOO_THROW_INVALID_OPERATION_EXCEPTION("isConnected not supported yet for ibverbs transport");
}
...
{
    GLOO_DEBUG(self_.str(), "->", peer_.str(), ": handleCompletion id=", wc->wr_id, " opcode=", wc->opcode);
    ...
    if (wc->opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
        ...
        auto slot = wc->imm_data;
        GLOO_ENFORCE_EQ(
            wc->status, IBV_WC_SUCCESS, "Memory region recv for slot ", slot, ": ", ibv_wc_status_str(wc->status));
        ...
        GLOO_DEBUG(self_.str(), "->", peer_.str(), ": handleCompletion id=", wc->wr_id, " opcode=IBV_WC_SEND");

        auto slot = wc->wr_id;
        GLOO_ENFORCE_EQ(
            wc->status, IBV_WC_SUCCESS, "Memory region send for slot ", slot, ": ", ibv_wc_status_str(wc->status));
...
void Pair::send(Buffer* buffer, size_t offset, size_t length, size_t roffset)
{
    ...
    auto send = [this, buffer, offset, length, roffset](struct ibv_mr peer) mutable {
        struct ibv_sge list;
        ...
        GLOO_DEBUG("send Buffer async slot=",
                   wr.wr_id,
                   " peer_addr=",
                   peer.addr,
                   " remote_addr=",
    ...
    if (buffer->peerMr_ == nullptr) {
        recvMemoryRegion(lock, buffer->slot_, [send, buffer](struct ibv_mr peer) mutable {
