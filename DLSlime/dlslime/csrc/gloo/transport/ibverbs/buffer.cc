    ...
    ex_(nullptr)
{
    ...
    if (mr_ == nullptr && errno == ENOMEM) {
        GLOO_ENFORCE(mr_ != nullptr, "ibv_reg_mr: ", strerror(errno), " (did you run into the locked memory limit?)");
    }
...
{
    {
        ...
        if (sendPending_ > 0) {
            GLOO_WARN("Destructing buffer with pending sends, sendPending_=", sendPending_);
        }
...
void Buffer::waitRecv()
{
    ...
    if (pair_->sync_) {
        ...
        while (recvCompletions_ == 0) {
            ...
            if (timeout != kNoTimeout && (std::chrono::steady_clock::now() - start) >= timeout) {
                pair_->signalIoFailure(GLOO_ERROR_MSG("Read timeout ", pair_->peer().str()));
    ...
    if (pair_->sync_) {
        ...
        if (sendCompletions_ == 0) {
            ...
            while (sendCompletions_ == 0) {
                pair_->pollCompletions();
                if (timeout != kNoTimeout && (std::chrono::steady_clock::now() - start) >= timeout) {
                    pair_->signalIoFailure(GLOO_ERROR_MSG("Send timeout ", pair_->peer().str()));
                    GLOO_ENFORCE(false, "Unexpected code path");
                }
            }
        }
