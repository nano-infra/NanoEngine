...
def main():
    ...
    # 只做 connect，支持单边发起（Smart Connect 机制会自动处理对端 Peer 创建和回连）
    # peer 惰性，两 broker 间一条链路
    print("Connecting A -> B ...")
    agent_a.connect(agent_b.client_addr, devices[0])

    # 也可以显示调用 B -> A，但现在不再必需（除非需要指定特定设备，否则 B 会用默认设备回连）
    # broker_b.connect(broker_a.client_addr, devices[-1])
