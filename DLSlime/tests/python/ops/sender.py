...
async def main(args):
    ...
    test_tensor = torch.arange(
        torch.prod(torch.tensor(test_shape)), dtype=torch.bfloat16, device="cuda"
    ).view(test_shape)
    ...
    buffer_tensor = torch.zeros(
        buffer_shape, device=test_tensor.device, dtype=test_tensor.dtype
    )
    engine.init_link(
        session_id, buffer_tensor, args.remote_host, args.remote_port, args.port
    )
    ...
    print(
        f"Total data size = {total_data_gb} GB, total time = {duration} s, {bandwidth=} GB/s"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", type=str, choices=["batch-send", "send"], default="batch-send"
    )
