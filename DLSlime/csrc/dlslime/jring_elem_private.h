...
__jring_enqueue_elems_32(struct jring* r, const uint32_t size, uint32_t idx, const void* obj_table, uint32_t n)
{
    unsigned int    i;
...
__jring_enqueue_elems_64(struct jring* r, uint32_t prod_head, const void* obj_table, uint32_t n)
{
    unsigned int    i;
...
static inline __attribute__((always_inline)) void
__jring_enqueue_elems_128(struct jring* r, uint32_t prod_head, const void* obj_table, uint32_t n)
{
    unsigned int      i;
    const uint32_t    size = r->size;
    uint32_t          idx  = prod_head & r->mask;
 ...
 */
static inline __attribute__((always_inline)) void
__jring_enqueue_elems(struct jring* r, uint32_t prod_head, const void* obj_table, uint32_t esize, uint32_t num)
{
...
static inline __attribute__((always_inline)) void
__jring_dequeue_elems_32(struct jring* r, const uint32_t size, uint32_t idx, void* obj_table, uint32_t n)
{
    unsigned int i;
    uint32_t*    ring = (uint32_t*)&r[1];
...
static inline __attribute__((always_inline)) void
__jring_dequeue_elems_64(struct jring* r, uint32_t prod_head, void* obj_table, uint32_t n)
{
    unsigned int   i;
    const uint32_t size = r->size;
    uint32_t       idx  = prod_head & r->mask;
...
static inline __attribute__((always_inline)) void
__jring_dequeue_elems_128(struct jring* r, uint32_t prod_head, void* obj_table, uint32_t n)
{
    unsigned int   i;
    const uint32_t size = r->size;
    uint32_t       idx  = prod_head & r->mask;
 ...
 */
static inline __attribute__((always_inline)) void
__jring_dequeue_elems(struct jring* r, uint32_t cons_head, void* obj_table, uint32_t esize, uint32_t num)
{
    /* 8B and 16B copies implemented individually to retain
     * the current performance.
     */
...
static inline __attribute__((always_inline)) void
__jring_wait_until_equal_32(volatile uint32_t* addr, uint32_t expected, int memorder)
{
...
static inline __attribute__((always_inline)) void __jring_update_tail(struct jring_headtail*           ht,
                                                                      uint32_t                         old_val,
                                                                      uint32_t                         new_val,
                                                                      uint32_t                         single,
 ...
 */
static inline __attribute__((always_inline)) unsigned int __jring_move_prod_head(struct jring*             r,
                                                                                 unsigned int              is_sp,
                                                                                 unsigned int              n,
                                                                                 enum jring_queue_behavior behavior,
                                                                                 uint32_t*                 old_head,
                                                                                 uint32_t*                 new_head,
    ...
    do {
        ...
        *new_head = *old_head + n;
        if (is_sp)
            ...
            success =
                __atomic_compare_exchange_n(&r->prod.head, old_head, *new_head, 0, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
 ...
 *     If behavior == JRING_QUEUE_FIXED, this will be 0 or n only.
 */
static inline __attribute__((always_inline)) unsigned int __jring_move_cons_head(struct jring*             r,
                                                                                 int                       is_sc,
                                                                                 unsigned int              n,
                                                                                 enum jring_queue_behavior behavior,
                                                                                 uint32_t*                 old_head,
 ...
 *   If behavior == jRING_QUEUE_FIXED, this will be 0 or n only.
 */
static inline __attribute__((always_inline)) unsigned int __jring_do_enqueue_elem(struct jring*             r,
                                                                                  ...
                                                                                  unsigned int*             free_space)
{
    ...
    n = __jring_move_prod_head(r, is_sp, n, behavior, &prod_head, &prod_next, &free_entries);
    if (n == 0)
        goto end;
 ...
 *     If behavior == jRING_QUEUE_FIXED, this will be 0 or n only.
 */
static inline __attribute__((always_inline)) unsigned int __jring_do_dequeue_elem(struct jring*             r,
                                                                                  void*                     obj_table,
                                                                                  unsigned int              esize,
                                                                                  unsigned int              n,
                                                                                  enum jring_queue_behavior behavior,
                                                                                  ...
                                                                                  unsigned int*             available)
{
    ...
    n = __jring_move_cons_head(r, (int)is_sc, n, behavior, &cons_head, &cons_next, &entries);
    if (n == 0)
        goto end;
