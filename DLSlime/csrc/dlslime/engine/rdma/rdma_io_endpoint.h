...
enum class IOContextState { FREE, PENDING, WAIT_TOKEN, POSTED, DONE };
...
public:
  ...
  std::shared_ptr<ReadWriteFuture> read(const std::vector<assign_tuple_t> &,
                                        void *stream);
  std::shared_ptr<ReadWriteFuture> write(const std::vector<assign_tuple_t> &,
                                         void *stream);
  std::shared_ptr<ReadWriteFuture>
  writeWithImm(const std::vector<assign_tuple_t> &, int32_t imm_data,
               void *stream);
...
private:
  ...
  int32_t dispatchTask(OpCode op_code, const std::vector<assign_tuple_t> &,
                       int32_t imm_data = 0, void *stream = nullptr);
