...
namespace dlslime {
int32_t RDMAChannel::init(std::shared_ptr<RDMAContext> ctx, size_t num_qp,
  ...
  for (int qpi = 0; qpi < num_qp; ++qpi) {
    ...
    if (max_inline_data == 0) {
      ...
      SLIME_ASSERT(max_inline_data <= 4096,
                   "inline data need to less than or equal to 4096");
    ...
    if (!qp_[qpi]) {
      SLIME_LOG_ERROR("[" << ctx_->device_name_ << "] Failed to create QP "
                          << qp_[qpi]->qp_num,
                      ": ", strerror(errno));
      return -1;
    }
    ...
    /* Get GID */
    if (ctx_->gidx_ != -1 &&
        ibv_query_gid(ctx_->ib_ctx_, 1, ctx_->gidx_, &ctx_->gid_)) {
...
int32_t RDMAChannel::connect(json remote_rdma_info_json) {
  SLIME_ASSERT(state == RDMAChannelState::Initialized,
               "Not Initialized or already connected");
  SLIME_ASSERT_EQ(local_rdma_info_.size(), remote_rdma_info_json.size(),
                  "Peer must have same QP Size.");
...
int32_t RDMAChannel::modify_qp_to_r2r() {
  for (int qpi = 0; qpi < local_rdma_info_.size(); qpi++) {
    ...
    attr.path_mtu = (enum ibv_mtu)std::min((uint32_t)remote_rdma_info_[qpi].mtu,
                                           (uint32_t)local_rdma_info_[qpi].mtu);
...
int64_t RDMAChannel::post_send_batch(int qpi, RDMAAssign *assign) {
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    ...
    wr[i].num_sge = 1;
    wr[i].imm_data =
        (i == batch_size - 1) ? assign->imm_data_ : UNDEFINED_IMM_DATA;
...
int64_t RDMAChannel::post_rc_oneside_batch(int qpi, RDMAAssign *assign) {
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    Assignment subassign = assign->batch_[i];
    struct ibv_mr *mr = memory_pool_->get_mr(subassign.mr_key);
    remote_mr_t remote_mr =
        memory_pool_->get_remote_mr(subassign.remote_mr_key);
    ...
    wr[i].imm_data =
        (i == batch_size - 1) ? assign->imm_data_ : UNDEFINED_IMM_DATA;
    wr[i].send_flags =
        (i % 32 == 0 || i == batch_size - 1) ? IBV_SEND_SIGNALED : 0;
    if (assign->is_inline_)
      wr[i].send_flags |= IBV_SEND_INLINE;
