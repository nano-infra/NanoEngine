...
RDMAMsgEndpoint::RDMAMsgEndpoint(std::shared_ptr<RDMAContext> ctx,
  ...
  recv_ctx_pool_ = reinterpret_cast<RecvContext *>(
      static_cast<char *>(raw_pool_ptr) + send_pool_size);

  // Initialize Signals
  for (int i = 0; i < SLIME_MAX_MSG_FIFO_DEPTH; ++i) {
    // Construct objects in place (placement new is good practice for POD-like
    // structs with constructors/std::shared_ptr) Adjusting access to properly
    // initialize
  ...
  for (size_t i = 0; i < SLIME_MAX_MSG_FIFO_DEPTH; ++i) {
    send_future_pool_.push_back(
        std::make_shared<SendFuture>(&(send_ctx_pool_[i])));
    recv_future_pool_.push_back(
        std::make_shared<RecvFuture>(&(recv_ctx_pool_[i])));
  }

  // Register Memory Regions (MR) upfront.
  // Dynamic registration during the datapath is expensive and should be
  // avoided.
  memory_pool->registerMemoryRegion(reinterpret_cast<uintptr_t>(dummy_),
                                    reinterpret_cast<uintptr_t>(dummy_),
                                    sizeof(int64_t));

  // Register the single large block
  memory_pool->registerMemoryRegion(reinterpret_cast<uintptr_t>(send_ctx_pool_),
                                    reinterpret_cast<uintptr_t>(send_ctx_pool_),
                                    total_size);

  // Calculate offset of remote_meta_info_ at runtime to avoid offsetof()
  // warning on non-POD types
  send_ctx_meta_offset_ =
      reinterpret_cast<uintptr_t>(&(send_ctx_pool_[0].remote_meta_info_)) -
      reinterpret_cast<uintptr_t>(send_ctx_pool_);

  SLIME_LOG_INFO(
      "Endpoint initialized. Send/Recv Pool Coalesced. Meta Offset: ",
      send_ctx_meta_offset_);
  ...
  // Initialize Rings. Size is double the depth to handle potential overflow
  // gracefully.
...
RDMAMsgEndpoint::~RDMAMsgEndpoint() {
  try {
    ...
    // Need to manually call destructors if they are non-trivial (which they
    // are, shared_ptr)
    for (int i = 0; i < SLIME_MAX_MSG_FIFO_DEPTH; ++i) {
    ...
    free(send_ctx_pool_);
    // free(recv_ctx_pool_); // Logic: recv_ctx_pool_ is part of the same block
    // as send_ctx_pool_
...
json RDMAMsgEndpoint::endpointInfo() const {
  ...
  json endpoint_info =
      json{{"meta_channel_info", meta_channel_->channelInfo()},
           {"data_channel_info", data_channel_->channelInfo()},
           {"remote_meta_base",
            {{"addr", base_ptr}, {"rkey", mr->rkey}, {"length", mr->length}}}};
...
void RDMAMsgEndpoint::connect(const json &remote_endpoint_info) {
  ...
  // Pre-post RECV requests for Meta Channel to handle incoming handshake
  // signals.
  for (int i = 0; i < SLIME_MAX_MSG_FIFO_DEPTH; ++i) {
    ...
    send_ctx->meta_recv_assign_.reset(
        OpCode::RECV, 0, batch, [send_ctx](int32_t status, int32_t imm) {
  ...
  // Pre-post RECV requests for Data Channel to handle completion signals (Imm
  // Data).
  for (int i = 0; i < SLIME_MAX_MSG_FIFO_DEPTH; ++i) {
    RecvContext *recv_ctx = &(recv_ctx_pool_[i]);
    for (size_t qpi = 0; qpi < num_qp_; ++qpi) {
      std::vector<Assignment> batch{Assignment(
          reinterpret_cast<uintptr_t>(dummy_), 0, 0, sizeof(int64_t))};

      recv_ctx->data_recv_assigns_[qpi].reset(
          OpCode::RECV, qpi, batch,
          [recv_ctx, qpi](int32_t status, int32_t imm) {
            if (status == 0) {
...
std::shared_ptr<SendFuture> RDMAMsgEndpoint::send(const chunk_tuple_t &chunk,
                                                  void *stream_handle) {
  ...
  uint64_t slot = send_slot_id_.fetch_add(1, std::memory_order_release) %
                  SLIME_MAX_MSG_FIFO_DEPTH;
  ...
  while (jring_enqueue_burst(send_buffer_ring_, (void **)&s_ctx, 1, nullptr) ==
         0) {
    cpu_relax();
  }
...
std::shared_ptr<RecvFuture> RDMAMsgEndpoint::recv(const chunk_tuple_t &chunk,
                                                  void *stream_handle) {
  auto data_ptr = std::get<0>(chunk);
  auto offset = std::get<1>(chunk);
  auto length = std::get<2>(chunk);
  ...
  uint32_t target_mask = (1 << num_qp_) - 1;
  uint64_t slot = recv_slot_id_.fetch_add(1, std::memory_order_release) %
                  SLIME_MAX_MSG_FIFO_DEPTH;
  ...
  while (jring_enqueue_burst(recv_buffer_ring_, (void **)&r_ctx, 1, nullptr) ==
         0) {
    cpu_relax();
  }
...
int32_t RDMAMsgEndpoint::process() { return sendProcess() + recvProcess(); }
...
int32_t RDMAMsgEndpoint::sendProcess() {
  ...
  int n = jring_dequeue_burst(send_buffer_ring_, send_new_burst_buf_,
                              BURST_SIZE, nullptr);
  if (n > 0) {
    work_done += n;
    for (int i = 0; i < n; ++i) {
  ...
  if (it != pending_send_queue_.end()) {
    ...
    case SendContextState::WAIT_META: {
      ...
      if (s_ctx->meta_arrived_flag_.val.load(std::memory_order_acquire)) {
        ...
        std::vector<Assignment> meta_batch{Assignment(
            reinterpret_cast<uintptr_t>(dummy_), 0, 0, sizeof(int64_t))};
        s_ctx->meta_recv_assign_.reset(
            OpCode::RECV, 0, meta_batch,
            [this, s_ctx](int32_t status, int32_t imm) {
        ...
        memory_pool_->registerRemoteMemoryRegion(
        ...
        for (size_t qpi = 0; qpi < num_qp_; ++qpi) {
          ...
          if (offset < total_len) {
            ...
            // Even if no data left, we must signal this QP to prevent receiver
            // hanging.
            current_len = 0;
            // Reset offset to 0 (valid range) for safety, though 0-len read
            // usually ignores address.
            offset = 0;
          }

          Assignment assign(s_ctx->local_meta_info_.view_.data_ptr,
                            s_ctx->remote_meta_info_.view_.data_ptr, offset,
                            offset, current_len);
          AssignmentBatch batch{assign};

          s_ctx->data_send_assigns_[qpi].reset(
              OpCode::WRITE_WITH_IMM, qpi, batch,
              [s_ctx, qpi](int32_t stat, int32_t imm_data) {
                s_ctx->signal->set_comm_done(qpi);
              },
              false);

          data_channel_->post_rc_oneside_batch(
              qpi, &(s_ctx->data_send_assigns_[qpi]));
        }
...
int32_t RDMAMsgEndpoint::recvProcess() {
  ...
  int n = jring_dequeue_burst(recv_buffer_ring_, recv_new_burst_buf_,
                              BURST_SIZE, nullptr);
  if (n > 0) {
    work_done += n;
    for (int i = 0; i < n; ++i) {
  ...
  if (it != pending_recv_queue_.end()) {
    ...
    case RecvContextState::INIT_SEND_META: {
      for (size_t qpi = 0; qpi < num_qp_; ++qpi) {
        std::vector<Assignment> batch{
            Assignment(reinterpret_cast<uintptr_t>(dummy_), 0, 0, 8)};
        r_ctx->data_recv_assigns_[qpi].reset(
            OpCode::RECV, qpi, batch,
            [r_ctx, qpi](int32_t status, int32_t imm) {
              if (status == 0) {
      ...
      uintptr_t local_meta_addr =
          reinterpret_cast<uintptr_t>(&(r_ctx->local_meta_info_));
      uintptr_t send_pool_base = reinterpret_cast<uintptr_t>(send_ctx_pool_);
      uint64_t local_offset = local_meta_addr - send_pool_base;

      // Calculate remote offset: slot * sizeof(SendContext) +
      // send_ctx_meta_offset_ usage of offsetof on non-standard layout type is
      // conditionally supported. using runtime calculated offset instead.
      uint64_t remote_offset =
          slot * sizeof(SendContext) + send_ctx_meta_offset_;

      Assignment assign(
          send_pool_base,
          r_ctx->remote_meta_key_, // Base address of remote SendPool
          remote_offset, local_offset, sizeof(meta_info_t));
      AssignmentBatch assign_batch{assign};

      r_ctx->meta_send_assign_.reset(OpCode::WRITE_WITH_IMM, 0, assign_batch,
                                     nullptr, true);
