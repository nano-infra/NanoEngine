...
typedef struct remote_mr {
  remote_mr() = default;
  remote_mr(uintptr_t addr, size_t length, uint32_t rkey)
      : addr(addr), length(length), rkey(rkey) {}
...
public:
  ...
  int registerMemoryRegion(const uintptr_t &mr_key, uintptr_t data_ptr,
                           uint64_t length);

  int unregisterMemoryRegion(const uintptr_t &mr_key);

  int registerRemoteMemoryRegion(const uintptr_t &mr_key, uintptr_t addr,
