...
int RDMAMemoryPool::registerMemoryRegion(const uintptr_t &mr_key,
                                         uintptr_t data_ptr, uint64_t length) {
  ...
  SLIME_LOG_DEBUG("Memory region: "
                  << mr_key << ", " << (void *)data_ptr << " -- "
                  << (void *)(data_ptr + length) << ", Device name: "
                  << pd_->context->device->dev_name << ", Length: " << length
                  << " (" << length / 1024 / 1024 << " MB)"
...
int RDMAMemoryPool::registerRemoteMemoryRegion(const uintptr_t &mr_key,
  ...
  SLIME_LOG_DEBUG("Remote memory region registered: " << mr_key << ", " << addr
                                                      << ", " << length << ", "
                                                      << rkey << ".");
  return 0;
}
...
int RDMAMemoryPool::registerRemoteMemoryRegion(const uintptr_t &mr_key,
                                               const json &mr_info) {
  std::unique_lock<std::mutex> lock(remote_mrs_mutex_);
  remote_mrs_[mr_key] = remote_mr_t(mr_info["addr"].get<uintptr_t>(),
                                    mr_info["length"].get<size_t>(),
                                    mr_info["rkey"].get<uint32_t>());
  SLIME_LOG_DEBUG("Remote memory region registered: " << mr_key << ", "
                                                      << mr_info << ".");
  return 0;
}
...
json RDMAMemoryPool::remote_mr_info() {
  ...
  for (auto &mr : remote_mrs_) {
    mr_info[mr.first] = {{"addr", mr.second.addr},
                         {"rkey", mr.second.rkey},
                         {"length", mr.second.length}};
  }
  return mr_info;
