...
int32_t RDMAIOEndpoint::dispatchTask(OpCode op_code,
  ...
  while (req_idx < total_reqs) {
    ...
    uint64_t slot = rw_slot_id_.fetch_add(1, std::memory_order_relaxed) %
                    SLIME_MAX_IO_FIFO_DEPTH;
    ...
    for (size_t qpi = 0; qpi < num_qp_; ++qpi) {
      ...
      assign.callback_ = [this, ctx, qpi, is_final_slot](int32_t status,
                                                         int32_t imm) {
        uint32_t old_mask =
            ctx->finished_qp_mask.fetch_or(1 << qpi, std::memory_order_acq_rel);
    ...
    while (jring_enqueue_burst(read_write_buffer_ring_, (void **)&ctx, 1,
                               nullptr) == 0) {
      _mm_pause();
    }
  }
...
RDMAIOEndpoint::RDMAIOEndpoint(std::shared_ptr<RDMAContext> ctx,
  ...
  SLIME_LOG_INFO("Initializing RDMA IO Endpoint. QPs: ", num_qp,
                 ", Token Bucket: ", SLIME_MAX_SEND_WR);
  ...
  if (posix_memalign(&raw_rw_ctx, 64,
                     sizeof(ReadWriteContext) * SLIME_MAX_IO_FIFO_DEPTH) != 0) {
    throw std::runtime_error(
        "Failed to allocate memory for ReadWriteContext pool");
  }
  read_write_ctx_pool_ = static_cast<ReadWriteContext *>(raw_rw_ctx);

  void *raw_recv_ctx = nullptr;
  if (posix_memalign(&raw_recv_ctx, 64,
                     sizeof(ImmRecvContext) * SLIME_MAX_IO_FIFO_DEPTH) != 0) {
    free(raw_rw_ctx);
    throw std::runtime_error(
        "Failed to allocate memory for ImmRecvContext pool");
  }
  ...
  for (size_t i = 0; i < SLIME_MAX_IO_FIFO_DEPTH; ++i) {
    read_write_future_pool_.push_back(
        std::make_shared<ReadWriteFuture>(&(read_write_ctx_pool_[i])));
    imm_recv_future_pool_.push_back(
        std::make_shared<ImmRecvFuture>(&(imm_recv_ctx_pool_[i])));
  }
...
RDMAIOEndpoint::read(const std::vector<assign_tuple_t> &assign, void *stream) {
  int32_t slot_id = dispatchTask(OpCode::READ, assign, 0, stream);
  return read_write_future_pool_[slot_id];
}

std::shared_ptr<ReadWriteFuture>
RDMAIOEndpoint::write(const std::vector<assign_tuple_t> &assign, void *stream) {
  int32_t slot_id = dispatchTask(OpCode::WRITE, assign, 0, stream);
  return read_write_future_pool_[slot_id];
}

std::shared_ptr<ReadWriteFuture>
RDMAIOEndpoint::writeWithImm(const std::vector<assign_tuple_t> &assign,
                             int32_t imm_data, void *stream) {
  int32_t slot_id =
      dispatchTask(OpCode::WRITE_WITH_IMM, assign, imm_data, stream);
  return read_write_future_pool_[slot_id];
}
...
std::shared_ptr<ImmRecvFuture> RDMAIOEndpoint::immRecv(void *stream) {
  ...
  uint64_t slot = recv_slot_id_.fetch_add(1, std::memory_order_relaxed) %
                  SLIME_MAX_IO_FIFO_DEPTH;
  ImmRecvContext *ctx = &imm_recv_ctx_pool_[slot];

  // Bind stream to the signal (safe even if signal is already set/posted)
  // Note: We do NOT reset signal here, as Worker might have already posted it
  // and it could complete any microsecond. Worker is responsible for resetting
...
int32_t RDMAIOEndpoint::readWriteProcess() {
  ...
  int n_rw = jring_dequeue_burst(read_write_buffer_ring_, burst_buf_,
                                 IO_BURST_SIZE, nullptr);
  if (n_rw > 0) {
  ...
  while (!pending_rw_queue_.empty()) {
    ...
    bool has_token = true;
    for (int qpi = 0; qpi < num_qp_; ++qpi) {
      if (token_bucket_[qpi].load(std::memory_order_acquire) <
          ctx->assigns_[qpi].batch_.size()) {
        has_token = false;
        break;
      }
    }
    ...
    for (size_t qpi = 0; qpi < num_qp_; ++qpi) {
      token_bucket_[qpi].fetch_sub(ctx->assigns_[qpi].batch_.size(),
...
int32_t RDMAIOEndpoint::immRecvProcess() {
  ...
  // Also respect ring buffer limit (don't overwrite unconsumed slots) ->
  // handled by user flow usually (blocking on future) But safely: posted - user
  // should not exceed FIFO_DEPTH.
  ...
  while (posted_cnt < target_post) {
    ...
    // Safety check: Don't lap the user by more than FIFO Depth (unlikely given
    // window << depth) If posted is too far ahead of user, we stop.
  ...
  // Drain ring just in case (legacy cleanup), though we don't push to it
  // anymore. jring_dequeue_burst(imm_recv_buffer_ring_, burst_buf_,
  // IO_BURST_SIZE, nullptr);
