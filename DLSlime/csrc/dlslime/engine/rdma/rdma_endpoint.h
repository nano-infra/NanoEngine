...
public:
  RDMAEndpoint(std::shared_ptr<RDMAContext> ctx, size_t num_qp,
               std::shared_ptr<RDMAWorker> worker = nullptr);

  RDMAEndpoint(std::string dev_name = "", int32_t ib_port = 1,
               std::string link_type = "RoCE", size_t num_qp = 1,
               std::shared_ptr<RDMAWorker> worker = nullptr);
  ...
  int32_t registerOrAccessMemoryRegion(uintptr_t mr_key, uintptr_t ptr,
                                       uintptr_t, size_t length);
  int32_t registerOrAccessRemoteMemoryRegion(uintptr_t ptr, json mr_info);

  // TwoSide Primitive
  std::shared_ptr<SendFuture> send(const chunk_tuple_t &chunk,
                                   void *stream_handler);
  std::shared_ptr<RecvFuture> recv(const chunk_tuple_t &chunk,
                                   void *stream_handler);

  // OneSide Primitive
  std::shared_ptr<ReadWriteFuture>
  read(const std::vector<assign_tuple_t> &assign, void *stream);
  std::shared_ptr<ReadWriteFuture>
  write(const std::vector<assign_tuple_t> &assign, void *stream);
  std::shared_ptr<ReadWriteFuture>
  writeWithImm(const std::vector<assign_tuple_t> &assign, int32_t imm_data,
               void *stream);
