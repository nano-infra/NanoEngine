...
int64_t RDMAContext::init(const std::string &dev_name, uint8_t ib_port,
  ...
  if (SLIME_MAX_RD_ATOMIC > int{device_attr.max_qp_init_rd_atom})
    SLIME_ABORT("MAX_RD_ATOMIC (" << SLIME_MAX_RD_ATOMIC
                                  << ") > device max RD ATOMIC ("
                                  << device_attr.max_qp_init_rd_atom
                                  << "), please set SLIME_MAX_RD_ATOMIC env "
  ...
  if (port_attr.state == IBV_PORT_DOWN) {
    ibv_close_device(ib_ctx_);
    SLIME_ABORT("Device " << dev_name << ", Port " << int{ib_port_}
                          << "is DISABLED.");
  }
  ...
  if (port_attr.link_layer == IBV_LINK_LAYER_INFINIBAND) {
    ...
    if (SLIME_GID_INDEX > 0)
      ...
      gidx_ = ibv_find_sgid_type(ib_ctx_, ib_port_,
                                 ibv_gid_type_custom::IBV_GID_TYPE_ROCE_V2,
                                 AF_INET);
    if (gidx_ < 0) {
  ...
  SLIME_LOG_INFO("Creating CQ with depth: "
                 << actual_cq_depth << " (Requested: " << SLIME_MAX_CQ_DEPTH
