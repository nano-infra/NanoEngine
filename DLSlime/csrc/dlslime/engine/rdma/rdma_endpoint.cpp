...
RDMAEndpoint::RDMAEndpoint(std::shared_ptr<RDMAContext> ctx, size_t num_qp,
                           std::shared_ptr<RDMAWorker> worker) {
  ctx_ = ctx;
  if (not ctx_)
    SLIME_ABORT("No NIC Resources");
  memory_pool_ = std::make_shared<RDMAMemoryPool>(ctx);
  worker_ = worker ? worker
                   : GlobalWorkerManager::instance().get_default_worker(
                         socketId(ctx_->device_name_));
...
RDMAEndpoint::RDMAEndpoint(std::string dev_name, int32_t ib_port,
                           std::string link_type, size_t num_qp,
                           std::shared_ptr<RDMAWorker> worker)
    : RDMAEndpoint(GlobalContextManager::instance().get_context(
                       dev_name, ib_port, link_type),
                   num_qp, worker) {}
...
void RDMAEndpoint::connect(const json &remote_endpoint_info) {
  for (auto &item : remote_endpoint_info["mr_info"].items()) {
    memory_pool_->registerRemoteMemoryRegion(
        item.value()["mr_key"].get<uintptr_t>(), item.value());
  }
...
void RDMAEndpoint::shutdown() {
  ...
  // Manually cancel all pending futures to unblock waiting threads (e.g. Client
  // destructor)
  if (io_endpoint_) {
...
int32_t RDMAEndpoint::registerOrAccessMemoryRegion(uintptr_t mr_key,
                                                   uintptr_t ptr,
                                                   uintptr_t offset,
                                                   size_t length) {
...
int32_t RDMAEndpoint::registerOrAccessRemoteMemoryRegion(uintptr_t ptr,
                                                         json mr_info) {
...
std::shared_ptr<SendFuture> RDMAEndpoint::send(const chunk_tuple_t &chunk,
                                               void *stream_handler) {
  return msg_endpoint_->send(chunk, stream_handler);
}

std::shared_ptr<RecvFuture> RDMAEndpoint::recv(const chunk_tuple_t &chunk,
                                               void *stream_handler) {
...
RDMAEndpoint::read(const std::vector<assign_tuple_t> &assign, void *stream)
...
std::shared_ptr<ReadWriteFuture>
RDMAEndpoint::write(const std::vector<assign_tuple_t> &assign, void *stream) {
  return io_endpoint_->write(assign, stream);
}

std::shared_ptr<ReadWriteFuture>
RDMAEndpoint::writeWithImm(const std::vector<assign_tuple_t> &assign,
