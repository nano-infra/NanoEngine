...
ReduceRangeFunction
genLocalReduceFunction(const BufferVector& in, const BufferVector& out, size_t elementSize, ReductionFunction fn)
{
    if (in.size() > 0) {
        if (in.size() == 1) {
...
BroadcastRangeFunction genLocalBroadcastFunction(const BufferVector& out)
{
    return [&out](size_t offset, size_t length) {
        for (size_t i = 1; i < out.size(); i++) {
            memcpy(
                static_cast<uint8_t*>(out[i]->ptr) + offset, static_cast<const uint8_t*>(out[0]->ptr) + offset, length);
        }
    };
}
...
void allreduce(const detail::AllreduceOptionsImpl& opts)
{
    ...
    const auto reduceInputs     = genLocalReduceFunction(in, out, opts.elementSize, opts.reduce);
          ...
          BroadcastRangeFunction              broadcastOutputs)
{
    ...
    GLOO_ENFORCE(context->getPair(recvRank),
                 "missing connection between rank " + std::to_string(context->rank) + " (this process) and rank "
                     + std::to_string(recvRank));
    GLOO_ENFORCE(context->getPair(sendRank),
                 "missing connection between rank " + std::to_string(context->rank) + " (this process) and rank "
                     + std::to_string(sendRank));
    ...
    const size_t maxSegmentBytes = opts.elementSize * std::max((size_t)1, opts.maxSegmentSize / opts.elementSize);
    ...
    const size_t numSegments =
        roundUp(std::max((totalBytes + (maxSegmentBytes - 1)) / maxSegmentBytes, (size_t)context->size * 2),
                (size_t)context->size);
    ...
    const size_t segmentBytes       = roundUp((totalBytes + numSegments - 1) / numSegments, opts.elementSize);
    ...
    auto computeReduceScatterOffsets = [&](size_t i) {
        ...
        result.sendOffset =
            ((((context->rank + 1) * numSegmentsPerRank) + i) * segmentBytes) % (numSegments * segmentBytes);
        result.recvOffset =
            ((((context->rank + 2) * numSegmentsPerRank) + i) * segmentBytes) % (numSegments * segmentBytes);
        ...
        result.sendLength = std::min((ssize_t)segmentBytes, (ssize_t)totalBytes - (ssize_t)result.sendOffset);
        result.recvLength = std::min((ssize_t)segmentBytes, (ssize_t)totalBytes - (ssize_t)result.recvOffset);
    ...
    auto computeAllgatherOffsets = [&](size_t i) {
        ...
        result.sendOffset =
            ((((context->rank) * numSegmentsPerRank) + i) * segmentBytes) % (numSegments * segmentBytes);
        result.recvOffset =
            ((((context->rank + 1) * numSegmentsPerRank) + i) * segmentBytes) % (numSegments * segmentBytes);
           ...
           BroadcastRangeFunction              broadcastOutputs)
{
    ...
    std::vector<struct group> groups;
    {
        ...
        for (const size_t groupSize : groupSizePerStep) {
            ...
            group.myChunkOffset = group.bufferOffset + (groupRank * group.chunkLength);
            group.myChunkLength = std::min(
                size_t(group.chunkLength),
                size_t(std::max(int64_t(0), int64_t(group.bufferLength) - int64_t(groupRank * group.chunkLength))));
    ...
    for (const auto& group : groups) {
        bufferLength = std::max(bufferLength, group.ranks.size() * group.chunkLength);
    }

    // Allocate scratch space to receive data from peers.
    const size_t                              bufferSize = bufferLength * elementSize;
    std::unique_ptr<uint8_t[]>                buffer(new uint8_t[bufferSize]);
    std::unique_ptr<transport::UnboundBuffer> tmp = context->createUnboundBuffer(buffer.get(), bufferSize);
    ...
    for (size_t step = 0; step < groups.size(); step++) {
        ...
        for (size_t i = 0; i < group.ranks.size(); i++) {
            ...
            tmp->recv(src, slot, i * group.chunkLength * elementSize, group.myChunkLength * elementSize);
        }
        ...
        for (size_t i = 0; i < group.ranks.size(); i++) {
            ...
            const size_t currentChunkOffset = group.bufferOffset + i * group.chunkLength;
            const size_t currentChunkLength =
                std::min(size_t(group.chunkLength),
                         size_t(std::max(int64_t(0), int64_t(group.bufferLength) - int64_t(i * group.chunkLength))));
            ...
            if (step == 0) {
                reduceInputs(currentChunkOffset * elementSize, currentChunkLength * elementSize);
            }
            out->send(dst, slot, currentChunkOffset * elementSize, currentChunkLength * elementSize);
        }
        ...
        if (step == 0) {
            reduceInputs(group.myChunkOffset * elementSize, group.myChunkLength * elementSize);
        }
        ...
        for (size_t i = 0; i < group.ranks.size(); i++) {
            ...
            opts.reduce(static_cast<uint8_t*>(out->ptr) + (group.myChunkOffset * elementSize),
                        static_cast<const uint8_t*>(out->ptr) + (group.myChunkOffset * elementSize),
                        static_cast<const uint8_t*>(tmp->ptr) + (i * group.chunkLength * elementSize),
                        group.myChunkLength);
        }
    }
    ...
    // chunks as we receive them from our peers during the allgather phase.
    {
        ...
        broadcastOutputs(group.myChunkOffset * elementSize, group.myChunkLength * elementSize);
    }
    ...
    for (auto it = groups.rbegin(); it != groups.rend(); it++) {
        ...
        for (size_t i = 0; i < group.ranks.size(); i++) {
            ...
            const size_t currentChunkLength =
                std::min(size_t(group.chunkLength),
                         size_t(std::max(int64_t(0), int64_t(group.bufferLength) - int64_t(i * group.chunkLength))));
            out->recv(src, slot, currentChunkOffset * elementSize, currentChunkLength * elementSize);
        }
        ...
        for (size_t i = 0; i < group.ranks.size(); i++) {
            ...
            out->send(dst, slot, group.myChunkOffset * elementSize, group.myChunkLength * elementSize);
        }
        ...
        for (size_t i = 0; i < group.ranks.size(); i++) {
            ...
            const size_t currentChunkLength =
                std::min(size_t(group.chunkLength),
                         size_t(std::max(int64_t(0), int64_t(group.bufferLength) - int64_t(i * group.chunkLength))));
            broadcastOutputs(currentChunkOffset * elementSize, currentChunkLength * elementSize);
        }
    }
}
