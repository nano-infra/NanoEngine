...
public:
    AllreduceRing(const std::shared_ptr<Context>& context,
        ...
        Algorithm(context), ptrs_(ptrs), count_(count), bytes_(count_ * sizeof(T)), fn_(fn)
    {
        ...
        recvNotificationBuf_  = rightPair->createRecvBuffer(notificationSlot, &dummy_, sizeof(dummy_));
    }
