...
struct AllreduceOptionsImpl {
    ...
    explicit AllreduceOptionsImpl(const std::shared_ptr<Context>& context):
        context(context), timeout(context->getTimeout()), algorithm(UNSPECIFIED)
    {
    }
...
public:
    ...
    explicit AllreduceOptions(const std::shared_ptr<Context>& context): impl_(context) {}
    ...
    void setInputs(T** ptrs, size_t len, size_t elements)
    {
        ...
        for (size_t i = 0; i < len; i++) {
            impl_.in.push_back(impl_.context->createUnboundBuffer(ptrs[i], elements * sizeof(T)));
        }
    }
    ...
    void setOutputs(T** ptrs, size_t len, size_t elements)
    {
        ...
        for (size_t i = 0; i < len; i++) {
            impl_.out.push_back(impl_.context->createUnboundBuffer(ptrs[i], elements * sizeof(T)));
        }
    }
