...
UnboundBuffer::UnboundBuffer(const std::shared_ptr<Context>& context, void* ptr, size_t size):
    ...
    shareableNonOwningPtr_(this)
{
    ...
    if (mr == nullptr && errno == ENOMEM) {
        GLOO_ENFORCE(mr != nullptr, "ibv_reg_mr: ", strerror(errno), " (did you run into the locked memory limit?)");
    }

    GLOO_ENFORCE(mr != nullptr, "ibv_reg_mr: ", strerror(errno), " ptr=", ptr, " size=", size);
...
void UnboundBuffer::handleCompletion(int rank, struct ibv_wc* wc)
{
    if (wc->opcode & IBV_WC_RECV) {
        ...
        GLOO_DEBUG("send complete sendPending=", sendPending_, " sendCompletions=", sendCompletions_);
    }
    else {
...
bool UnboundBuffer::waitRecv(int* rank, std::chrono::milliseconds timeout)
{
    ...
    if (timeout == kNoTimeout || timeout == kUnsetTimeout) {
        ...
        if (!done) {
            ...
            auto msg = GLOO_ERROR_MSG("Timed out waiting ", timeout.count(), "ms for recv operation to complete");
    ...
    if (sendCompletions_ == 0) {
        ...
        if (timeout == kNoTimeout || timeout == kUnsetTimeout) {
            ...
            if (!done) {
                ...
                // reacquire.
                lock.unlock();
                auto msg = GLOO_ERROR_MSG("Timed out waiting ", timeout.count(), "ms for send operation to complete");
                context_->signalException(msg);
                throw ::gloo::IoException(msg);
            }
        }
...
void UnboundBuffer::send(int dstRank, uint64_t slot, size_t offset, size_t nbytes)
{
...
void UnboundBuffer::recv(int srcRank, uint64_t slot, size_t offset, size_t nbytes)
{
    // Default the number of bytes to be equal to the number
    // of bytes remaining in the buffer w.r.t. the offset.
    if (nbytes == kUnspecifiedByteCount) {
...
void UnboundBuffer::recv(std::vector<int> srcRanks, uint64_t slot, size_t offset, size_t nbytes)
{
    // Default the number of bytes to be equal to the number
    // of bytes remaining in the buffer w.r.t. the offset.
    if (nbytes == kUnspecifiedByteCount) {
