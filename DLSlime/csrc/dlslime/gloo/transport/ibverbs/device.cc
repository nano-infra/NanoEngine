...
std::shared_ptr<::gloo::transport::Device> CreateDevice(const struct attr& constAttr)
{
    ...
    if (attr.name.empty()) {
        ...
        for (auto i = 0; i < devices.size(); i++) {
            GLOO_DEBUG("found candidate device ", devices[i]->name, " dev=", devices[i]->dev_name);
    ...
    GLOO_INFO("Using ibverbs device=", attr.name, " port=", attr.port, " index=", attr.index);
    ...
    if (!context) {
        GLOO_THROW_INVALID_OPERATION_EXCEPTION("Unable to find device named: ", attr.name);
    }
    return std::make_shared<Device>(attr, context);
}

Device::Device(const struct attr& attr, ibv_context* context):
    attr_(attr), pciBusID_(infinibandToBusID(attr.name)), hasNvPeerMem_(hasNvPeerMem()), context_(context)
{
    int rv;
...
{
    return std::shared_ptr<transport::Context>(new ibverbs::Context(shared_from_this(), rank, size));
}
