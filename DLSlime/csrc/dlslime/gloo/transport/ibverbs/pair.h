...
class Pair: public ::gloo::transport::Pair {
    ...
    static constexpr auto kCompletionQueueCapacity     = kRecvCompletionQueueCapacity + kSendCompletionQueueCapacity;
...
public:
    explicit Pair(int rank, const std::shared_ptr<Device>& dev, std::chrono::milliseconds timeout);
    ...
    virtual std::unique_ptr<::gloo::transport::Buffer> createSendBuffer(int slot, void* ptr, size_t size) override;

    virtual std::unique_ptr<::gloo::transport::Buffer> createRecvBuffer(int slot, void* ptr, size_t size) override;
...
protected:
    ...
    std::map<int, std::deque<std::function<void(struct ibv_mr)>>> recvMemoryRegionCallbacks_;
    ...
    void recvMemoryRegion(std::unique_lock<std::mutex>& lock, int slot, std::function<void(struct ibv_mr)> callback);
