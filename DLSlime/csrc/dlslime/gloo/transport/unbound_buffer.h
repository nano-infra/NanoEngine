...
public:
    ...
    {
        return waitRecv(std::chrono::duration_cast<std::chrono::milliseconds>(deadline - clock::now()));
    }
    ...
    {
        return waitSend(std::chrono::duration_cast<std::chrono::milliseconds>(deadline - clock::now()));
    }
    ...
    virtual void send(int dstRank, uint64_t slot, size_t offset = 0, size_t nbytes = kUnspecifiedByteCount) = 0;

    virtual void recv(int srcRank, uint64_t slot, size_t offset = 0, size_t nbytes = kUnspecifiedByteCount) = 0;

    virtual void
    recv(std::vector<int> srcRanks, uint64_t slot, size_t offset = 0, size_t nbytes = kUnspecifiedByteCount) = 0;
};
