...
def test_all_to_all_offsets():
    ...
    if rank == 0:
        ...
        print("Note: Running 'No Offsets' with packed input 'x' might fail or behave unexpectedly if API expects padded input when offsets are missing.")
        print("Adjusting input for No Offsets test to match MAX_BS padding if needed...")
        
    # Wait, the user's script uses 'x' (my_count, MSG_SIZE) for calling no_offsets?
    # If no offsets are provided, does it expect (MAX_BS * world_size) or just broadcast?
    # Let's trust the user's script logic for now, or see if it fails.
    # Actually, looking at the code provided in the prompt:
    # res_no_offsets = buffer.all_to_all_ll(x, is_transpose=False)
    # The user says "Test without offsets". 
    # If 'x' is smaller than full buffer, maybe it works if it just takes the first elements?
    
    # Trying to run as provided.
    try:
        res_no_offsets = buffer.all_to_all_ll(x, is_transpose=False)
        ...
        print(f"No Offsets test failed: {e}")
    ...
    my_count = counts[rank].item()
    x_alltoall = torch.zeros((world_size * my_count, MSG_SIZE), dtype=DTYPE, device="cuda")
    ...
    if rank == 0:
        ...
        for src in range(world_size):
            ...
            assert torch.all(actual_slice == expected_val), f"Transpose data mismatch from rank {src}"
