...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fbs_files = [
        "../dlengine-proto/sequence.fbs",
        "../dlengine-proto/packet.fbs",
    ];
    ...
    // different path on PATH — after such a change run ``cargo clean -p
    // dlengine-router`` (or simply ``rm -rf target/debug/build/dlengine_router-*``)
    // once so the generated sources are regenerated by the new tool.
