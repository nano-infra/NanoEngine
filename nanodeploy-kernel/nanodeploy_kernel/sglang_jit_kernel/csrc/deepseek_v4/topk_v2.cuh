...
PLAN_KERNEL void topk_plan(const uint32_t* __restrict__ seq_lens,
                           ...
                           const uint32_t static_cluster_threshold)
{
    ...
    if (static_cluster_threshold > 0) {
        if (tx == 0)
            s_threshold = static_cluster_threshold;
    }
    else if (batch_size <= kMinBatchSize) {
        if (tx == 0)
            s_threshold = kMax2PassLength;  // always prefer cluster
    }
    else {
...
#pragma unroll
            for (uint32_t i = 0; i < kNumCandidates; ++i) {
                ...
                if (accum > kCandidates[j].max_batch_size)
                    break;
    ...
    for (uint32_t i = tx; i < N; i += kBlockSize) {
        if (i + kNumClusters < N)
            metadata[1 + i].has_next = true;
    }
    // Zero-fill the first kNumClusters sentinel slots that got no valid entry.
    if (tx < kNumClusters && tx >= N)
        metadata[1 + tx] = {0, 0, false};
...
topk_fused_transform(const __grid_constant__ TopKParams params)
{
    ...
    if (seq_len <= K) {
        ...
        if (cluster_rank != 0)
            return;  // only first rank do the stage-2
        Large::transform(transform, ws, smem);
    }
}
