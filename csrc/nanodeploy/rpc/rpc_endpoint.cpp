...
public:
    ...
    Serializer() { buffer.reserve(4096); }
    ...
    // 修改：使用 BlockContext::BlockLocationList 类型
    void write_block_locations(const BlockContext::BlockLocationList& vec) {
        write<size_t>(vec.size());
    ...
    // 修改：使用 BlockContext::SpBlockTable 类型
    void write_sp_block_table(const BlockContext::SpBlockTable& map) {
        write<size_t>(map.size());
        for (const auto& kv : map) {
            write<int>(kv.first);
            // kv.second 是 BlockIdList，它继承自 vector<int>，可以向上转型传给 write_vector_basic
            write_vector_basic(kv.second);
        }
    }
};
...
public:
    ...
    Deserializer(void* data, size_t size) {
        if (!data) throw std::runtime_error("Deserializer received null pointer");
    ...
    template <typename T>
    T read() {
        if (ptr + sizeof(T) > end) throw std::runtime_error("Buffer underflow reading primitive");
        T val;
    ...
    std::string read_string() {
        ...
        if (ptr + len > end) throw std::runtime_error("Buffer underflow reading string");
    ...
    std::vector<T> read_vector_basic() {
        size_t len = read<size_t>();
        if (ptr + len * sizeof(T) > end) throw std::runtime_error("Buffer underflow reading vector");
    ...
    // 修改：返回 BlockContext::BlockLocationList 类型
    BlockContext::BlockLocationList read_block_locations() {
        size_t len = read<size_t>();
        BlockContext::BlockLocationList vec;
        vec.reserve(len);
    ...
    // 修改：返回 BlockContext::SpBlockTable 类型
    BlockContext::SpBlockTable read_sp_block_table() {
        size_t len = read<size_t>();
        BlockContext::SpBlockTable map;
        for (size_t i = 0; i < len; ++i) {
            int key = read<int>();
            // 先读出基础的 vector<int>
            auto vec_base = read_vector_basic<int>();
            // 构造 BlockIdList (继承了 vector 的构造函数)
            BlockContext::BlockIdList val(vec_base.begin(), vec_base.end());
...
// ==========================================================
// 具体的 Sequence 和 BlockContext 序列化逻辑
// ==========================================================
...
int32_t RpcEndpoint::serialize_for_decode() {
    // Decode 阶段通常也可以用全量序列化，或根据需求优化为增量
