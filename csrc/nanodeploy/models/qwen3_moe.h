...
public:
                          ...
                          torch::Tensor              seq_lens     = {})
    {
        ...
        if (is_decode && kv_cache && handler) {
            ...
            attn_output = at::scaled_dot_product_attention(q,
                                                           k_sdpa,
                                                           v_sdpa,
                                                           /*attn_mask=*/{},
                                                           /*dropout_p=*/0.0,
...
public:
    Qwen3MoeMLP(const core::ModelConfig& config): core::Module()
    {
        ...
        gate_up_proj_ =
            std::make_unique<layers::MergedColumnParallelLinear<Q>>(hidden_size, 2 * intermediate_size, false);
        down_proj_ = std::make_unique<layers::RowParallelLinear<Q>>(intermediate_size, hidden_size, false);
...
public:
    ...
    torch::Tensor forward(torch::Tensor hidden_states, deep_ep::Buffer* ep_buffer)
    {
        ...
        if (is_prefill) {
            ...
            auto res_ll = ep_buffer->low_latency_dispatch(hidden_states,
                                                          topk_ids,
        ...
        if (use_fp8_) {
            ...
            // auto intermediate = torch::empty({num_local_experts, max_tokens, moe_inter * 2},
            // torch::kBFloat16).to(recv_x.device());
        ...
        auto res_comb   = ep_buffer->low_latency_combine(expert_out,  // Pass 3D or 2D?
                                                       topk_ids,
                                                       topk_weights,
                                                       src_info,
                                                       layout_range,
...
public:
    Qwen3MoeDecoderLayer(const core::ModelConfig&                 config,
                         int                                      layer_idx,
                         std::shared_ptr<layers::RotaryEmbedding> rotary):
        core::Module()
    {
                                                     ...
                                                     deep_ep::Buffer*           ep_buffer)
    {
        ...
        // 2. Attention
        auto attn_out = self_attn_->forward(positions, normed, kv_cache, handler, slot_mapping, layer_idx_);
...
public:
    Qwen3MoeModel(const core::ModelConfig& config): core::Module()
    {
        ...
        rotary_ =
            std::make_shared<layers::RotaryEmbedding>(head_dim, config.max_position_embeddings, config.rope_theta);
