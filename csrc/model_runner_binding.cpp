...
PrefillMetadata ModelRunnerCore::prepare_prefill(std::vector<std::shared_ptr<Sequence>>& seqs, bool is_dummy)
{
    ...
    for (auto& seq : seqs) {
        ...
        int num_cached_blocks =
            seq->num_cached_blocks();  // This might need adjustment if num_cached_blocks logic differs
...
DecodeMetadata ModelRunnerCore::prepare_decode(std::vector<std::shared_ptr<Sequence>>& dp_seqs, bool is_dummy)
{
    ...
    int end_pos   = start_pos + sp_valid_request_counts[attn_sp_rank];
    ...
    meta.context_lens_for_attn_start_pos = start_pos;
    meta.context_lens_for_attn_end_pos   = end_pos;
...
void bind_model_runner(py::module& m)
{
    ...
    py::class_<DecodeMetadata>(m, "DecodeMetadata")
        ...
        .def_readonly("context_lens_for_attn_start_pos", &DecodeMetadata::context_lens_for_attn_start_pos)
        .def_readonly("context_lens_for_attn_end_pos", &DecodeMetadata::context_lens_for_attn_end_pos);
