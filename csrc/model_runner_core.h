...
struct DecodeMetadata {
    ...
    int32_t context_lens_for_attn_start_pos;
    int32_t context_lens_for_attn_end_pos;
};
...
public:
    ...
    ModelRunnerCore(std::string engine_id,
                    int         rank,
                    int         world_size,
                    int         max_num_seqs,
                    int         max_num_send_seqs,
        ...
        max_num_send_seqs(max_num_send_seqs),
        max_num_recv_seqs(max_num_recv_seqs),
        block_size(block_size),
        attn_sp_rank(attn_sp_rank),
        attn_sp_world_size(attn_sp_world_size)
    {
    }
