                                               ...
                                               int           hidden_size)
{
    ...
    for (int e = 0; e < num_local_experts; ++e) {
        ...
        if (count > 0) {
            ...
            m_indices.slice(0, aligned_offset, aligned_offset + aligned_count).fill_(group_idx);
            group_idx++;
        }
...
// FP8 version that takes pre-quantized input (from dispatch_normal_fp8)
torch::Tensor MoeExpertOps::compute_contiguous_fp8(torch::Tensor                           input_fp8,
                                                   ...
                                                   int                                     hidden_size)
{
    int  num_tokens        = input_fp8.size(0);
    auto device            = input_fp8.device();
    int  num_local_experts = gate_up.first.size(0);
    int  moe_inter_2       = gate_up.first.size(1);
    int  top_k             = topk_idx.size(1);

    if (num_tokens == 0) {
        return torch::zeros({0, hidden_size}, torch::TensorOptions().dtype(torch::kBFloat16).device(device));
    }
    ...
    if (used_experts.empty() || total_aligned_rows == 0) {
        return torch::zeros({num_tokens, hidden_size}, torch::TensorOptions().dtype(torch::kBFloat16).device(device));
    }

    // Allocate aligned FP8 and scale tensors
    int  num_k_tiles   = input_fp8.size(1) / 128;
    auto aligned_x_fp8 = torch::zeros({total_aligned_rows, input_fp8.size(1)}, input_fp8.options());
    auto aligned_x_scale_base =
        torch::zeros({num_k_tiles, total_aligned_rows}, torch::TensorOptions().dtype(torch::kFloat32).device(device));
    auto aligned_x_scale = aligned_x_scale_base.t();  // TMA layout
    ...
    for (int e = 0; e < num_local_experts; ++e) {
        ...
        if (count > 0) {
            ...
            aligned_x_fp8.slice(0, aligned_offset, aligned_offset + count) =
                input_fp8.index_select(0, token_indices_tensor);

            aligned_x_scale.slice(0, aligned_offset, aligned_offset + count) =
                input_scales.index_select(0, token_indices_tensor);
    ...
    auto gateup_out =
        torch::empty({total_aligned_rows, moe_inter_2}, torch::TensorOptions().dtype(torch::kBFloat16).device(device));
    ...
    auto aligned_down_out =
        torch::empty({total_aligned_rows, hidden_size}, torch::TensorOptions().dtype(torch::kBFloat16).device(device));

    DeepGemmOps::m_grouped_fp8_gemm_nt_contiguous(
        {act_out_fp8, act_out_scale}, {down_w_sel, down_s_sel}, aligned_down_out, m_indices);

    auto output =
        torch::zeros({num_tokens, hidden_size}, torch::TensorOptions().dtype(torch::kBFloat16).device(device));
    auto weights_gpu = topk_weights.to(torch::kFloat32).to(device);
