...
DispatchResult DeepEpOps::dispatch_normal(torch::Tensor hidden_states,
                                          torch::Tensor topk_ids,
                                          torch::Tensor topk_weights,
                                          int           num_experts,
                                          int           expert_alignment)
{
    auto buffer = get_deep_ep_context().get_buffer();
    auto config = *(get_deep_ep_context().get_config());
...
torch::Tensor DeepEpOps::combine_normal(torch::Tensor expert_output, const DispatchHandle& handle)
{
...
DispatchResultFP8 DeepEpOps::dispatch_normal_fp8(torch::Tensor x_fp8,
                                                 ...
                                                 int           expert_alignment)
{
    ...
    // Ensure x_fp8 is actually FP8
    TORCH_CHECK(x_fp8.scalar_type() == torch::kFloat8_e4m3fn,
                "dispatch_normal_fp8 expects FP8 input, got ",
                x_fp8.scalar_type());

    auto x_flat = x_fp8.view({-1, x_fp8.size(-1)});
    ...
    DispatchResultFP8 result;
    result.handle.use_internode    = use_internode;
    result.handle.hidden_size      = x_flat.size(1);
    result.handle.config           = config;
    result.handle.prev_event       = prev_event;
    result.handle.is_token_in_rank = is_token_in_rank;

    if (use_internode) {
        // Internode dispatch with FP8 + scales
        auto dispatch_ret = buffer->internode_dispatch(x_flat.contiguous(),
                                                       x_scales,  // Pass scales!
                                                       topk_ids,
                                                       topk_weights,
                                                       num_tokens_per_rank,
                                                       num_tokens_per_rdma_rank,
        ...
        result.recv_x                                 = std::get<0>(dispatch_ret);
        result.recv_x_scales                          = std::get<1>(dispatch_ret).value();
        result.recv_topk_idx                          = std::get<2>(dispatch_ret);
        result.recv_topk_weights                      = std::get<3>(dispatch_ret);
        result.handle.recv_rdma_channel_prefix_matrix = std::get<7>(dispatch_ret).value();
        result.handle.recv_rdma_rank_prefix_sum       = std::get<8>(dispatch_ret);
        ...
        // Intranode dispatch with FP8 + scales
        auto dispatch_ret = buffer->intranode_dispatch(x_flat.contiguous(),
                                                       x_scales,  // Pass scales!
                                                       topk_ids,
                                                       topk_weights,
                                                       num_tokens_per_rank,
...
LowLatencyDispatchResult DeepEpOps::dispatch_low_latency(torch::Tensor hidden_states,
                                                         torch::Tensor topk_ids,
                                                         torch::Tensor topk_weights,
...
torch::Tensor DeepEpOps::combine_low_latency(torch::Tensor expert_output, const LowLatencyDispatchHandle& handle)
{
    auto buffer = get_deep_ep_context().get_buffer();
