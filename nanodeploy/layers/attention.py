    ...
    def forward(
        ...
        if context.is_prefill:
            ...
            if sp_size > 1:
                ...
                block_tables = context.block_tables.view(-1, context.block_tables.shape[-1])[: context.attention_compute_bs]
            else:
                q = q[: context.attention_compute_bs]
                context_lens_for_attn = context.context_lens_for_attn[
                    : context.attention_compute_bs
                ...
                block_tables = context.block_tables.view(-1, context.block_tables.shape[-1])[: context.attention_compute_bs]
            # logger.info(f"{q.unsqueeze(1).shape=};{context_lens_for_attn.shape=};{block_tables.shape=};{k_cache.shape=};{context.attention_compute_bs=};{context.block_tables.shape=}")
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        scale,
        num_kv_heads,
        v_head_dim=None,  # Optional for backward compatibility
