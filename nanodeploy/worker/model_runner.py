...
from nanodeploy.worker.context import set_context, get_context, reset_context
...
class ModelRunner(NanoVLLMModelRunner):
    ...
    def prepare_dummy(self):
        ...
        block_tables = torch.tensor([[1]], dtype=torch.int32, pin_memory=True).cuda(non_blocking=True)
        set_context(False, slot_mapping=slot_mapping, context_lens=context_lens, block_tables=block_tables, is_dummy=True)
