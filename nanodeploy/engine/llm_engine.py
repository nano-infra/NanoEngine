...
class LLMEngine:
    ...
    def step(self):
        ...
        if not is_prefill:
            # Skip log-only state traversal when INFO output is disabled.
            if logger.isEnabledFor(logging.INFO):
                ...
                if self.log_decode_step_detail:
                    # Per-SP-rank seq_lens: [dp_idx][sp_idx] -> list of seq
                    # lens on that GPU.
                    sp_seq_lens = [
                        [
                            [
                                len(seq)
                                for seq in filtered_dp_sp_seqs[
                                    dp_idx * sp_size + sp_idx
                                ]
                            ]
                            for sp_idx in range(sp_size)
                        ]
                    ...
                    logger.info(
                        {
                            ...
                            "itl": f"{itl:.2f}ms",
                            "sch_ovhd": (
                                f"{(sch_end - sch_begin) * 1000:.2f}ms"
                            ),
                            "post_sch_ovhd": (
                                f"{(post_sch_end - post_sch_begin) * 1000:.2f}ms"
                            ),
                    ...
                    min_free_blocks = (
                        min(flat_free_blocks) if flat_free_blocks else 0
                    )
                    max_kv_util_pct = (
                        100.0
                    ...
                    logger.info(
                        {
                            ...
                            "max_sp_batch_size": (
                                max(flat_sp_batch_sizes)
                                if flat_sp_batch_sizes
                                else 0
                            ),
                            "min_free_blocks": min_free_blocks,
                            "max_kv_util_pct": f"{max_kv_util_pct:.2f}",
                            "sp_size_hist_global": sp_size_hist_global,
                            "waiting_head_blocks_sum": sum(
                                waiting_head_blocks
                            ),
                            "waiting_total_blocks_sum": sum(
