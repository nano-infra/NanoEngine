...
Vision-Language encoder engine for NanoDeploy, shipped as the `dlengine.vl` subpackage. Implements an **EP-separated** (Encoder-Prefill separated) architecture where the vision encoder runs as a standalone service, producing embeddings that are transferred to LLM prefill engines via RDMA.
...
- `examples/test_vl.py` — full multimodal request through dlengine-router
