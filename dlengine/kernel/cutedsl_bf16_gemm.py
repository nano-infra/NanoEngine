...
# Kimi K3 decode GEMM shapes covered by the specialized kernel.
...
def use_cutedsl_bf16_gemm(m: int, n: int, k: int) -> bool:
    """Return whether the specialized kernel covers this GEMM shape."""
    if m <= 0:
...
def _tgv_bf16_gemm_out_fake(
    ...
    return None
def blackwell_bf16_linear(
    x: torch.Tensor,
    weight: torch.Tensor,
    bias: torch.Tensor | None = None,
) -> torch.Tensor:
    """Dispatch covered Blackwell BF16 shapes to TGV, otherwise use PyTorch."""
    ...
    if (
        ...
        and use_cutedsl_bf16_gemm(rows, weight.shape[0], weight.shape[1])
    ):
        out = cutedsl_bf16_gemm(
            x.detach().reshape(-1, x.shape[-1]),
            weight.detach(),
...
def cutedsl_bf16_gemm(
    ...
    return _tgv_bf16_gemm_run(x, weight, bias)
def cutedsl_bf16_gemm_out(
    ...
    _tgv_bf16_gemm_out_run(x, weight, out, bias)
    return out
