...
def _jit_hicache_module(*, element_size: int, unroll: int, block_quota: int) -> Module:
    ...
    return load_jit(
        "hicache",
        *args,
        cuda_files=["kvcacheio/hicache.cuh"],
        cuda_wrappers=[
...
def can_use_hicache_jit_kernel(
    *,
    ...
    if element_size % 128 != 0:
        logger.warning("Unsupported element_size=%s for JIT HiCache kernel", element_size)
        return False
    try:
        _jit_hicache_module(
        ...
        logger.warning("Failed to load JIT HiCache kernel: %s", exc)
        return False
...
def transfer_hicache_one_layer_mla(
    ...
    *,
    ...
    module.launch_one_mla(cache_dst, indices_dst, cache_src, indices_src)
