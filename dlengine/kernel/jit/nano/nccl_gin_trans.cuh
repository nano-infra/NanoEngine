...
static void broadcast_nccl_id(int rank, int world, ncclUniqueId* id)
{
    ...
    if (rank == 0) {
        ...
        if (fd < 0)
            throw std::runtime_error("could not connect to rank0 bootstrap");
...
__global__ void init_u32(uint32_t* ptr, int n, uint32_t base)
{
    ...
    if (i < n)
        ptr[i] = base + static_cast<uint32_t>(i);
}

__global__ void
hbm_to_remote_dram_kernel(ncclDevComm dev_comm, ncclWindow_t dev_win, ncclWindow_t host_win, int nbytes, int peer)
{
    ...
    gin.put(ncclTeamWorld(dev_comm),
            peer,
            host_win,
            0,
            dev_win,
...
__global__ void
remote_dram_to_hbm_kernel(ncclDevComm dev_comm, ncclWindow_t host_win, ncclWindow_t dev_win, int nbytes, int peer)
{
    ncclGin                            gin{dev_comm, 0};
    ncclGinBarrierSession<ncclCoopCta> bar{ncclCoopCta(), gin, ncclTeamTagWorld(), 0};
    bar.sync(ncclCoopCta(), cuda::memory_order_acquire, ncclGinFenceLevel::Relaxed);
    ...
    // the destination.
    gin.get(ncclTeamWorld(dev_comm),
            peer,
            host_win,
            0,
            dev_win,
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < count; ++i)
        h_buf[i] = 0xdead0000u + static_cast<uint32_t>(rank);
    init_u32<<<(count + 255) / 256, 256>>>(
        static_cast<uint32_t*>(d_buf), count, 0x10000000u + static_cast<uint32_t>(rank) * 0x10000u);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (out[i] != expected) {
            fprintf(
                stderr, "rank %d remote-DRAM->HBM mismatch i=%d got=0x%x expected=0x%x\n", rank, i, out[i], expected);
            ok_get = false;
            break;
        }
    }
