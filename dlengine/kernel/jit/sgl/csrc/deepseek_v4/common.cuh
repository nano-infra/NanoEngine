...
{
    const auto& [compress_plan,
                 write_plan,
                 seq_lens,
                 extend_lens,  // pointers
                 batch_size,
                 num_tokens,
                 compress_ratio,
                 is_overlap  // values
...
{
    const auto& [compress_ptr,
                 write_ptr,
                 seq_lens_ptr,
                 extend_lens_ptr,  // pointers
                 batch_size,
                 num_tokens,
                 compress_ratio,
                 is_overlap  // values
    ...
    if (!use_cuda_graph)
        return PlanResult{compress_counter, write_counter};
    constexpr auto kInvalid     = static_cast<uint32_t>(-1);
...
inline PlanResult plan_prefill(const tvm::ffi::TensorView extend_lens,
                               ...
                               const bool use_cuda_graph)
{
    ...
    if (!is_cuda)
        return plan_prefill_host(params, use_cuda_graph);
