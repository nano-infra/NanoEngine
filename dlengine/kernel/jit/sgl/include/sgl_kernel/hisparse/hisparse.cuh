...
inline void launch_identity_remap_slots(
    const int32_t* in_slots, int32_t* out_slots, const int32_t* num_real_reqs, int max_slots, cudaStream_t stream)
{
    if (max_slots <= 0) {
        return;
    }
    ...
    identity_remap_slots_kernel<<<blocks, kThreads, 0, stream>>>(in_slots, out_slots, num_real_reqs, max_slots);
}
