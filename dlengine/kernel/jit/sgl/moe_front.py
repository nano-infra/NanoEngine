...
The fp32 merged output preserves router selection semantics while the epilogue
casts only the latent slice to bf16.
"""
...
# Above this token count callers may choose a different front strategy.
...
# it is the default vector width; issuing the cast after the
...
def get_front_strategy(num_tokens: int, device) -> str:
    """Return the configured front strategy for this exact workload.

    GEMM algorithm changes make the merged/overlap crossover non-monotonic in
    M. Therefore strategy tables are exact-match only. Other shapes keep
