...
@triton.jit
def _score_kernel(
    prefix, bank, cw, scores, nvb: tl.constexpr, eps: tl.constexpr,
    stride_pm, stride_bm, stride_bb, stride_sm,
    ...
    for h0 in tl.static_range(0, H, BLOCK_H):
        ...
        if row < nvb:
            value = tl.load(bank + token * stride_bm + row * stride_bb + h).to(tl.float32)
        else:
...
@triton.jit
def _combine_kernel(
    prefix, bank, scores, output, nvb: tl.constexpr,
    stride_pm, stride_bm, stride_bb, stride_sm, stride_om,
    BLOCK_H: tl.constexpr, MAX_ROWS: tl.constexpr,
):
    token = tl.program_id(0)
...
def fused_attention_residual(
    ...
    _score_kernel[(tokens, nvb + 1)](
        prefix, bank, combined_score_weight, scores, nvb, eps,
        prefix.stride(0), bank.stride(0), bank.stride(1), scores.stride(0),
        H=hidden, BLOCK_H=_BLOCK_H, num_warps=8,
    )
    output = torch.empty_like(prefix)
    _combine_kernel[(tokens, hidden // _BLOCK_H)](
        prefix, bank, scores, output, nvb,
        prefix.stride(0), bank.stride(0), bank.stride(1), scores.stride(0), output.stride(0),
