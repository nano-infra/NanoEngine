...
CUDA header 放在 JIT/kernel tree 下，命名为 `hisparse.cuh`，再加一个小的 Python wrapper 调用。风格和现有 `dlengine/dlengine/kernel/jit/sgl` 下引入的 SGLang JIT kernels 保持一致。

## 文件级集成点

实现时按现有 DLEngine 目录边界放置职责：

- `dlengine/dlengine/csrc/scheduler/`
...
- `dlengine/dlengine/models/deepseek_v2/`
...
1. 增加 config flags 和 validation；
2. 把 HiSparse 做成独立 cache-plan component，例如 `deepseek_mla_cache_plan(use_hisparse=True)`，而不是现有 MLA/indexer cache plan 的 runtime overlay；
3. 增加 `HiSparseContext` 和 dummy/no-op remap hooks；完整 `HiSparseCoordinator` host tier 延后到 Phase 2；
4. 给 `DecodeGraphRunner` 增加 persistent HiSparse tensors；
5. 扩展 `BatchContext`，增加 `hisparse_slots`、`hisparse_slot_mapping`、`hisparse_num_real_reqs`；
6. 扩展 `InputPreparer.prepare_decode_bytes`，从显式 C++ aux metadata 构建 `hisparse_slots`；
7. `DeepseekV2Attention` decode 中，`enable_hisparse` 时把 top-k 交给 coordinator；
8. FP8 KV store 使用 remapped device-buffer slots；
9. dummy-prefill 下 KV/indexer cache 填 deterministic zeros，保证可复现；
