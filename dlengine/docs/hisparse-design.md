...
JIT kernels already under `dlengine/dlengine/kernel/jit/sgl`.
...
1. Add config flags and validation.
2. Add HiSparse as a separate cache-plan component, for example `deepseek_mla_cache_plan(use_hisparse=True)`, instead of treating it as a runtime overlay on the existing MLA/indexer plan.
3. Add `HiSparseContext` with dummy/no-op remap hooks; defer the full `HiSparseCoordinator` host tier to Phase 2.
4. Add persistent HiSparse tensors to `DecodeGraphRunner`.
5. Extend `BatchContext` with `hisparse_slots`, `hisparse_slot_mapping`, and
   `hisparse_num_real_reqs`.
6. Extend `InputPreparer.prepare_decode_bytes` to build `hisparse_slots` from
   explicit C++ aux metadata.
7. In `DeepseekV2Attention` decode, route top-k through coordinator when
   `enable_hisparse`.
8. Use remapped device-buffer slots for FP8 KV store.
9. Fill dummy-prefill KV/indexer cache with deterministic zeros for reproducibility.
