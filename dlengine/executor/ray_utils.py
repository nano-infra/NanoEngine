...
def _ray_head_node_ip(all_nodes: list[dict]) -> str | None:
    for node in all_nodes:
        resources = node.get("Resources", {}) or {}
        if node.get("Alive", True) and "node:__internal_head__" in resources:
...
def get_available_nodes_with_master_first(
    ...
    if cleaned_host in {"localhost", "127.0.0.1"}:
        resolved_master_ip = _ray_head_node_ip(all_nodes)
        if resolved_master_ip is None:
            raise RuntimeError("No alive Ray node found to resolve localhost")
    else:
