#!/usr/bin/env python3
"""Compare official DeepSeek-V4 and DLEngine debug tensor dumps."""
...
def _nano(nano_dir: Path, stem: str, mode: str) -> torch.Tensor | None:
    files = sorted(nano_dir.glob(f"dlengine_rank*_{stem}.pt"))
    if not files:
        return None
...
def main() -> None:
    ...
    parser.add_argument("--dlengine-dir", required=True)
    parser.add_argument("--layer", type=int, default=0)
    args = parser.parse_args()

    official_dir = Path(args.official_dir)
    nano_dir = Path(args.dlengine_dir)
    print(
