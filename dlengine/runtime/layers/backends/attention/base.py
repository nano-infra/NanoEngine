"""Shared GQA attention scaffolding.

``GqaAttentionBase`` owns the plumbing common to every dense-GQA attention
backend — KV-cache writes, HiSparse sliding-window handling, cached-prefill
gather, FlashInfer paged metadata/plan caching, and SDPA reference kernels — and
exposes three override hooks for the actual attend kernels:

- ``_attend_prefill_paged``  : paged cached-prefill kernel (return ``None`` to
  fall back to gather + varlen)
- ``_attend_prefill_varlen`` : ragged prefill kernel
- ``_attend_decode_paged``   : paged decode kernel (return ``None`` to fall back
  to the no-cache SDPA path)

Concrete backends (``generic`` = SDPA, ``fa2`` = FlashAttention-2,
``flashinfer`` = FlashInfer paged) implement only these hooks, so each owns its
kernel calls instead of a single impl branching on ``use_*`` flags.
"""
...
class GqaAttentionBase(AttentionBase):
    """GQA attention layer with shared plumbing and kernel override hooks."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if attention_type != "GQA":
            raise ValueError(
                f"{type(self).__name__} only supports GQA attention, got "
                f"{attention_type!r}."
            )
        # NSA/DSA-specific arguments are accepted (the shared DeepSeek/GLM
        # constructor passes them uniformly) but the GQA backends do not index.
    ...
    # ------------------------------------------------------------------
    # Kernel override hooks (implemented by concrete backends)
    # ------------------------------------------------------------------

    def _attend_prefill_paged(
        self, q, k_cache, v_cache, block_tables, num_seqs, context
    ):
        """Paged cached-prefill kernel. Return ``None`` to fall back to
        gather + varlen."""
        return None

    def _attend_prefill_varlen(self, q, k, v, context):
        """Ragged (varlen) prefill kernel over concatenated K/V."""
        raise NotImplementedError

    def _attend_decode_paged(
        self, q, k_cache, v_cache, block_tables, context_lens, bs, ntps, context
    ):
        """Paged decode kernel. Return ``None`` to fall back to the no-cache
        SDPA path."""
        return None

    # ------------------------------------------------------------------
    # Forward orchestration (shared)
    # ------------------------------------------------------------------
    ...
    def forward(
        ...
        hot_k_cache, hot_v_cache = self.hisparse_k_cache, self.hisparse_v_cache
        ...
        if context.is_prefill:
            return self._prefill(q, k, v, k_cache, v_cache, hot_k_cache, hot_v_cache)
        return self._decode(q, k, v, k_cache, v_cache)

    def _prefill(self, q, k, v, k_cache, v_cache, hot_k_cache, hot_v_cache):
        context = get_batch_context()
        ...
        if (
            ...
            return self._attend_prefill_varlen(q, k, v, context)
        ...
        if context.block_tables is not None and k_cache.numel() and v_cache.numel():
            ...
            if _has_cached_prefill(context):
                paged = self._attend_prefill_paged(q, k_cache, v_cache, bt, num_seqs, context)
    ...
    def _decode(self, q, k, v, k_cache, v_cache):
        context = get_batch_context()
        ntps = context.num_tokens_per_seq
        ...
        if k_cache.numel() and v_cache.numel():
            paged = self._attend_decode_paged(
                q, k_cache, v_cache, block_tables, context_lens, bs, ntps, context
            )
            if paged is not None:
                return paged
        ...
        raise RuntimeError(
            f"{type(self).__name__} does not support paged KV decode; select "
