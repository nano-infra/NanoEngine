...
def _k3_output_norm_kernel(
    ...
    mask = cols < dim
    xv = tl.load(x + batch * sx_b + head * sx_h + cols, mask=mask, other=0.0).to(
        tl.float32
    )
    gv = tl.load(gate + batch * sg_b + head * sg_h + cols, mask=mask, other=0.0).to(
        tl.float32
    )
    wv = tl.load(weight + cols, mask=mask, other=0.0).to(tl.float32)
    inv = tl.rsqrt(tl.sum(xv * xv, axis=0) / dim + eps)
    tl.store(
        out + batch * so_b + head * so_h + cols,
        xv * inv * wv * tl.sigmoid(gv),
        mask=mask,
    )
