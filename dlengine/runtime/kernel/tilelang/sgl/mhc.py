...
# Apache-2.0; see ../sglang_jit_kernel/LICENSE for the full text.
#
# Modifications:
#   - ``is_arch_support_pdl`` re-imported from our existing vendored
#     sglang_jit_kernel slice instead of upstream ``sglang.jit_kernel``.
#   - The two single-call dependencies on ``sglang.srt.*`` are inlined
#     as no-op stubs (used only on the NSA-prefill CP round-robin path,
#     which DSV4 decode never hits).
...
from dlengine.runtime.kernel.jit.sgl.utils import is_arch_support_pdl


def is_nsa_prefill_cp_round_robin_split() -> bool:
    """Stub: NSA-prefill CP path is not used in dlengine's DSV4 decode."""
    return False


def strict_contiguous(x: torch.Tensor) -> torch.Tensor:
    """Stub: matches upstream's ``.contiguous()``-with-extra-checks helper."""
    return x.contiguous()
