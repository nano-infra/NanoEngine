...
//
// The fused path preserves fp32 routing logits and casts only the latent slice.
//
...
// routed-scaling, winners in descending routed-score order.
...
#pragma unroll
            for (uint32_t i = 0; i < kVecSize; ++i) {
                if (active[i])
                    atomicAdd(&smem.histogram[bin[i]], 1);
            }
            __syncthreads();
            ...
            if (tx < kRadixLanes) {
                ...
                if (lane_id == 31)
                    smem.warp_sum[0][warp_id] = warp_inc;
...
#pragma unroll
            ...
            if (round + 1 < T::kRadixRounds && tx < kRadixLanes)
                zero.store(smem.histogram, tx);
        }
    }
...
#pragma unroll
        ...
        if (params.apply_scale)
            wv = wv * params.routed_scaling_factor;
...
struct FusedFrontEpilogueKernel {
    static void run(const tvm::ffi::TensorView merged,  // [M, E + latent] fp32, row-dense
                    ...
                    bool                       cast_first)
    {
        ...
        RuntimeCheck(E_.unwrap() == sglang::kFGTNumExperts && K_.unwrap() == sglang::kFGTTopK
                         && topk == sglang::kFGTTopK,
                     "fused_front_epilogue is specialized for E=896, topk=16");
        RuntimeCheck(static_cast<int>(W_.unwrap()) == static_cast<int>(sglang::kFGTNumExperts) + latent,
        ...
        RuntimeCheck(merged.stride(0) % 4 == 0 && routed.stride(0) % 4 == 0,
        ...
        RuntimeCheck(cast_vec == 2 || cast_vec == 4 || cast_vec == 8,
