...
private:
    ...
    static SGL_DEVICE void reduce_impl(uint32_t num_vecs,  //
...
uint32_t choose_block_size(uint32_t num_threads)
{
    ...
    for (const uint32_t block_size : {128u, 256u, 512u}) {
        if (host::div_ceil(num_threads, block_size) <= kNumSM)
            return block_size;
    }
    return 1024u;
}
...
public:
    static Tensor run(CommunicatorRef ref, Tensor in_, std::string algo, std::variant<TensorView, bool> pull_arg)
    {
        ...
        if (algo == "1shot_push") {
            ...
            RuntimeCheck(
                nbytes <= data.push_bytes, "Input size ", nbytes, " exceeds push workspace size ", data.push_bytes);
        ...
        using enum PullMode;
        RuntimeCheck(
            nbytes <= data.pull_bytes, "Input size ", nbytes, " exceeds pull workspace size ", data.pull_bytes);
        ...
        const auto     cuda_memcpy = [&](void* dst, const void* src) {
            if constexpr (SGL_ARCH_HOPPER_OR_GREATER) {  // PDL memcpy is faster
                // only enable when batch size is small and aligned
        ...
        if (algo == "1shot_pull") {
            ...
            if (!use_graph)
                cuda_memcpy(out.data_ptr(), local_workspace);
        }
        return out;
    }
};
...
template<typename T, uint32_t kWorldSize, bool kUsePDL>
tvm::ffi::Tensor custom_all_reduce(CommunicatorRef                          comm,
