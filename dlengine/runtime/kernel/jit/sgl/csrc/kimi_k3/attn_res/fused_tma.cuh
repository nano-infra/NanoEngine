...
public:
    ...
    // Chunk slots in the shared-memory ring use double buffering.
    ...
    static_assert(kConsumerRegs == 0
                      || (kConsumerRegs % 8 == 0 && 24 <= kConsumerRegs && kConsumerRegs <= 256
...
SGL_DEVICE void
KimiK3AttnResTrait<kDim_, kNumBankRows_, kChunkRows_, kConsumerRegs_>::forward(const AttnResTMAParams& params,
                                                                               Smem*                   smem)
{
    ...
    if (warp_id >= kNumConsumerWarps) {  // producer warp (group); first warp works
        if constexpr (kConsumerRegs > 0)
            ::ptx::setmaxnreg_dec<kProducerRegs>();
...
#pragma unroll
                    for (uint32_t r = 0; r < an; ++r) {
                        ...
                        if (token == blockIdx.x && row == kNumRows)
                            PDLWaitPrimary<true>();
        ...
        if constexpr (kConsumerRegs > 0)
            ::ptx::setmaxnreg_inc<kConsumerRegs>();
...
#pragma unroll
            for (uint32_t si = 0; si < kSlicesPerGroup; ++si) {
                ::ptx::tcgen05_st_32x32b_x8(tmem_cw + si * kVecElems,
                                            reinterpret_cast<const uint32_t*>(&staged[si * kVecElems]));
            }
#pragma unroll
            for (uint32_t si = 0; si < kSlicesPerGroup; ++si) {
                const auto tile = si * kNumGroups + group;
...
#pragma unroll
            for (uint32_t si = 0; si < kSlicesPerGroup; ++si) {
                ::ptx::tcgen05_st_32x32b_x8(tmem_ow + si * kVecElems,
                                            reinterpret_cast<const uint32_t*>(&staged[si * kVecElems]));
            }
            ::ptx::tcgen05_wait_st();
        }
...
struct AttnResFusedTmaKernel {
    ...
    {
        return std::array<KernelFn, kMaxBankRows + 1>{nullptr,
                                                      attn_res_fused_pull_rs_kernel<Trait<I + 1>, kOccupancy>...};
    }
    ...
    {
        return std::array<KernelFn, kMaxBankRows + 1>{nullptr,
                                                      attn_res_fused_direct_ag_kernel<Trait<I + 1>, kOccupancy>...};
    }
    static constexpr auto kAgTable = make_ag_table(std::make_index_sequence<kMaxBankRows>{});

    static void run(const tvm::ffi::TensorView prefix_sum,
                    ...
                    bool                       write_prefix)
    {
        ...
        RuntimeCheck(1 <= nvb && nvb <= kMaxBankRows && nvb <= NB,
                     ...
                     " NB=",
                     NB);
        RuntimeCheck(!write_prefix || nvb < NB,
        ...
        [[maybe_unused]] static const bool attrs_set = [] {
            for (uint32_t i = 1; i <= kMaxBankRows; ++i) {
                RuntimeDeviceCheck(
                    cudaFuncSetAttribute(kTable[i], cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemBytes));
            }
            return true;
        }();
    ...
    static void run_pull_rs(CommunicatorRef                     ref,
                            ...
                            int64_t                             max_blocks)
    {
        ...
        RuntimeCheck(global_tokens == num_tokens * data.world_size,
                     "global tokens must equal local tokens * world size");
        ...
        const auto num_sm      = runtime::get_sm_count(device.unwrap().device_id);
        const auto grid        = std::min<int64_t>({static_cast<int64_t>(num_sm) * kOccupancy,
                                                    num_tokens,
                                                    max_blocks,
    ...
    static void run_direct_ag(CommunicatorRef            ref,
                              ...
                              bool                       write_prefix)
    {
        ...
        [[maybe_unused]] static const bool attrs_set = [] {
            for (uint32_t i = 1; i <= kMaxBankRows; ++i) {
                RuntimeDeviceCheck(
                    cudaFuncSetAttribute(kAgTable[i], cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemBytes));
            }
            return true;
        }();
