...
__global__ void reduce_scatter_pull_kernel(const __grid_constant__ Params params)
{
    ...
    const auto*    residual = kHasResidual ?
                                  params.residual + (params.residual_is_local ? 0 : params.rank * params.local_vecs * 16) :
                                  nullptr;
    for (uint32_t vid = tid; vid < params.local_vecs; vid += step) {
        vec_t vec;
...
struct SPCollectiveKernel {
    ...
    static sp_collective::Params make_params(const host::distributed::CommunicatorObj& data,
                                             TensorView                                input,
                                             TensorView                                output,
    ...
    static void reduce_scatter_res(CommunicatorRef           ref,
                                   TensorView                input,
                                   TensorView                output,
                                   std::optional<TensorView> residual,
                                   bool                      residual_is_local,
    ...
    static void all_gather(CommunicatorRef ref,
                           TensorView      input,
                           TensorView      output,
                           int64_t         ws_mc_base,
                           int64_t         num_blocks,
    ...
    static void all_gather_direct(CommunicatorRef ref,
                                  TensorView      input,
                                  TensorView      output,
    ...
    static void reduce_scatter_pull(CommunicatorRef           ref,
                                    TensorView                input,
                                    TensorView                output,
                                    std::optional<TensorView> residual,
                                    bool                      residual_is_local,
                                    ...
                                    int64_t                   block_size)
    {
        ...
        params.sem_mc     = reinterpret_cast<uint8_t*>(static_cast<uintptr_t>(sem_mc_ptr));
        const auto kernel = residual.has_value() ?
                                sp_collective::reduce_scatter_pull_kernel<kWorldSize, true, kUsePDL> :
