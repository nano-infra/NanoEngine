...
def _table(world_size: int, hidden_size: int, device: torch.device) -> Optional[dict]:
    ...
    if path not in _TABLES:
        candidate = path
        if not os.path.exists(candidate) and torch.cuda.get_device_capability(
            device
        ) == (10, 3):
            candidate = os.path.join(
                _CONFIG_DIR,
                f"world={world_size},H={hidden_size},device_name=NVIDIA_GB300.json",
            )
...
def register_comm(comm, *, pull_sem_mc_ptr: int = 0) -> None:
