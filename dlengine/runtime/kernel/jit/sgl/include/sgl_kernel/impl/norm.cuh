...
{
    if (!std::is_same_v<T, fp16_t> && !std::is_same_v<T, bf16_t>)
        return false;
    if (kDim <= 256) {
...
SGL_DEVICE AlignedVector<PackedFloat, N> apply_norm_impl(const AlignedVector<PackedFloat, N> input,
...
template<int64_t kDim, typename T>
SGL_DEVICE T
apply_norm_cta(const T& input, const T& weight, float eps, float* smem, uint32_t num_warps = blockDim.x / kWarpThreads)
{
