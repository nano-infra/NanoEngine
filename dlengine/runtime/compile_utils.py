...
All dlengine call sites go through :func:`maybe_compile` instead of calling
...
The switch can be set two ways (both checked at wrap time):
  * env var ``DLENGINE_DISABLE_COMPILE=1`` — useful before any config exists;
...
_COMPILE_DISABLED: bool = os.getenv("DLENGINE_DISABLE_COMPILE", "0") == "1"


def set_compile_disabled(disabled: bool) -> None:
    """Globally enable/disable dlengine's ``torch.compile`` wrappers.
