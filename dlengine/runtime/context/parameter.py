...
class ParameterContext(BaseContext):
    """Versioned local store for tensors pulled by the parameter-update path."""
    ...
    def pull_named_tensors_via_rdma(self, train_alias: str, manifest) -> None:
        ...
        if self.peer_context is None:
            raise RuntimeError("ParameterContext PeerAgentContext is not initialized")
...
def _apply_direct(
    ...
    *,
    ...
    """Pre-refactor direct-lookup path. Kept for fallback only."""
...
class ParameterUpdateEngine:
    """Coordinates RDMA parameter pulls, local storage, and model apply."""

    def __init__(self, model: torch.nn.Module, parameter_context: ParameterContext):
        self.model = model
        self.parameter_context = parameter_context
        self.weight_context = parameter_context
    ...
    def apply_context(self, *, sync: bool = True) -> dict[str, int]:
        return self.apply_named_tensors(
            self.parameter_context.named_tensors(), sync=sync
        )
    ...
    def pull_into_context(
        ...
        t0 = time.monotonic()
        self.parameter_context.pull_named_tensors_via_rdma(train_alias, manifest)
    ...
    def pull_and_apply(self, manifest_blob: bytes, train_alias: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.parameter_context.clear(release_mrs=True)

        stats = {**pull_stats, "apply_s": apply_s, **counts}
        logger.info("ParameterUpdateEngine.pull_and_apply: %s", stats)
        return stats


WeightContext = ParameterContext
WeightUpdateEngine = ParameterUpdateEngine

__all__ = [
    "ParameterContext",
    "ParameterUpdateEngine",
    "WeightContext",
    "WeightTensorEntry",
    "WeightUpdateEngine",
    "apply_named_tensors_in_place",
]
