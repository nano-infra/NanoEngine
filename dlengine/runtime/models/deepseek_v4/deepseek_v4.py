...
from dlengine.runtime.context.cache.plan import deepseek_v4_cache_plan
from dlengine.runtime.context.distributed import get_dist_context
from dlengine.runtime.kernel.jit.sgl import (
...
from dlengine.runtime.models.deepseek_v2.deepseek_v2 import DeepseekV2MLP
...
# Source (vendored under dlengine/runtime/kernel/jit/sgl):
...
try:
    from dlengine.runtime.models.deepseek_v4.index_kernels import (
...
if _DSV4_FUSED_KERNELS:
    try:
        from dlengine.runtime.models.deepseek_v4.compress_kernels import (
...
if _DSV4_FUSED_KERNELS:
    try:
        from dlengine.runtime.kernel.tilelang.sgl import mhc_pre as _SGL_MHC_PRE
    except Exception:
        _SGL_MHC_PRE = None
else:
...
def _debug_layer_enabled(layer_idx: int | None) -> bool:
    if layer_idx is None:
        return True
    layers = os.getenv("DLENGINE_DSV4_DEBUG_LAYERS", "0")
...
def _debug_dump(name: str, tensor: torch.Tensor, layer_idx: int | None = None) -> None:
    out_dir = os.getenv("DLENGINE_DSV4_DEBUG_DIR")
    if not out_dir or not torch.is_tensor(tensor):
        return
    rank = _debug_rank()
    rank_filter = os.getenv("DLENGINE_DSV4_DEBUG_RANK", "0")
    ...
    if is_dummy and os.getenv("DLENGINE_DSV4_DEBUG_SKIP_DUMMY", "1") != "0":
        return
    ...
    if is_prefill_run:
        if os.getenv("DLENGINE_DSV4_DEBUG_PREFILL_ONLY", "1") != "0":
            pass  # prefill mode is the default and is graph-safe
    else:
        # Decode dumping disabled by default. Only enable with
        # DLENGINE_DSV4_DEBUG_PREFILL_ONLY=0 and require eager mode
        # (no CUDAGraph) — caller must run with --enforce_eager true.
        if os.getenv("DLENGINE_DSV4_DEBUG_PREFILL_ONLY", "1") != "0":
            return
        decode_steps_env = os.getenv("DLENGINE_DSV4_DEBUG_DECODE_STEPS", "")
    ...
    max_tokens = int(os.getenv("DLENGINE_DSV4_DEBUG_MAX_TOKENS", "8"))
    ...
    file_path = path / f"dlengine_rank{rank}_{layer}_{name}{step_suffix}.pt"
    if os.getenv("DLENGINE_DSV4_DEBUG_ONCE", "1") != "0" and file_path.exists():
        return
    torch.save(
        {
            "name": name,
...
def _fp8_quant_dequant_inplace(x: torch.Tensor, block_size: int = 64) -> torch.Tensor:
    ...
    # reference's no-op semantics here: DLEngine's KV cache is FP8
...
class DeepseekV4HCProjector(nn.Module):
    ...
    DLEngine keeps the sequence dimension flattened, so tensors are shaped
    ...
    def forward(
        ...
        if (
            ...
            try:
                ...
                # dlengine's ``x`` is already that shape.
                post_mix, comb_mix, layer_input = _SGL_MHC_PRE(
                    ...
                    hc_post_mult_value=2.0,  # dlengine: post = 2 * sigmoid(...)
                ...
                # match dlengine's eager return contract.
                return (
                    layer_input,  # y
...
class DeepseekV4Attention(nn.Module):
    ...
    This deliberately uses DLEngine's existing paged GQA attention backend:
...
class DeepseekV4DecoderLayer(nn.Module):
    ...
    def _hc_post(
        ...
        # DLEngine form ``residual.unsqueeze(1)`` + ``sum(dim=2)``
