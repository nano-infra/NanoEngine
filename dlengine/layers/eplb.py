# modify from deepseek and sglang
import random
...
def compute_logical_to_rank_dispatch_physical_map(
    ...
    for layer_id in range(num_layers):
        for logical_expert_id in range(num_logical_experts):
            candidate_physical_expert_ids = logical_to_all_physical_raw(
...
class EPLBMetadata:
    ...
    def __post_init__(self):
        ...
        _, num_layers_3, num_logical_experts_2 = (
    ...
    def init(ep_size: int, num_routed_experts: int, num_hidden_layers: int):
        ...
        if weight_path is None:
            experts_statistic = (
                torch.arange(num_routed_experts, dtype=torch.int32, device="cuda")
                .flip(dims=(0,))
                .expand(num_hidden_layers, -1)
            )
        else:
            try:
                import json
                ...
                with open(weight_path, "r") as f:
                    experts_statistic = torch.tensor(
                        json.load(f), dtype=torch.float32, device="cuda"
                    )
            except Exception:
                raise RuntimeError(
                    f"Load eplb experts statistic data failed, path: {weight_path}"
                )
            target_shape = torch.Size([num_hidden_layers, num_routed_experts])
            assert (
                experts_statistic.shape == target_shape
            ), f"Shape of {weight_path} must be {target_shape}"
...
def init_global_eplb_metadata(
    ep_size: int, num_routed_experts: int, num_hidden_layers: int
):
    global _global_eplb_metadata
    assert _global_eplb_metadata is None
    _global_eplb_metadata = EPLBMetadata.init(
        ep_size=ep_size,
...
class EPLBDispatchInfo:
    ...
    def init_new(cls, ep_rank: int, layer_idx: int):
        ...
        return cls(
            ...
            partial_logical_to_all_physical_map=eplb_metadata.logical_to_all_physical_map[
                layer_idx, :
            ],
