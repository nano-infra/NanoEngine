...
class AnthropicError(Exception):
    ...
    def to_response(self):  # noqa: ANN201
        ...
        return JSONResponse(
            ...
            content={"type": "error", "error": {"type": self.err_type, "message": self.message}},
        )
...
def anthropic_to_openai_messages(body: dict) -> tuple[list[dict], Optional[list[dict]], Any]:
    ...
    for m in raw_messages:
        ...
        for block in content:
            ...
            if btype == "text":
                ...
                tool_results.append((block.get("tool_use_id"), _text_from_blocks(block.get("content"))))
                ...
                raise AnthropicError(400, "image content blocks are not supported by this text-only model")
        ...
        if role == "assistant":
            ...
            for tool_use_id, result_text in tool_results:
                messages.append({"role": "tool", "tool_call_id": tool_use_id, "content": result_text})
...
def build_sampling_params(body: dict) -> Any:
    ...
    if not isinstance(max_tokens, int) or max_tokens <= 0:
        raise AnthropicError(400, "'max_tokens' is required and must be a positive integer")
    temperature = body.get("temperature")
    if temperature is None:
        temperature = 1.0
    return SamplingParams(temperature=float(temperature), max_tokens=int(max_tokens), ignore_eos=False)
    ...
    *,
    ...
    for tc in tool_calls:
        content_blocks.append(
            {"type": "tool_use", "id": tc.id, "name": tc.function.name, "input": _tool_input(tc)}
        )
    if tool_calls:
            ...
            for tc in tool_calls:
                ...
                yield _sse(
                    "content_block_delta",
                    {
                        "type": "content_block_delta",
                        "index": index,
                        "delta": {"type": "input_json_delta", "partial_json": json.dumps(_tool_input(tc))},
                    },
                )
                yield _sse("content_block_stop", {"type": "content_block_stop", "index": index})
...
async def handle_messages(server: Any, request: Any, body: dict):  # noqa: ANN201
    ...
    monitor = server._spawn_disconnect_monitor(request, req)
    try:
        async for delta, gen in server.stream_text(req, sampling_params.max_tokens, stop=stop):
            text += delta
    except RuntimeError as e:
        return JSONResponse(
            status_code=500,
...
async def handle_count_tokens(server: Any, body: dict):  # noqa: ANN201
    ...
    use_tools = uses_tools(tools, tool_choice)
    prompt_ids, _ = server._encode_chat(messages, tools=tools if use_tools else None, tool_choice=tool_choice)
