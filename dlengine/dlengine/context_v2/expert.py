...
class ExpertContext(BaseContext):
    ...
    def reset(cls) -> None:
        """用于测试等场景重置上下文"""
        reset_expert_instance()


_EXPERT_CONTEXT: Optional[ExpertContext] = None


def get_expert_context() -> ExpertContext:
    global _EXPERT_CONTEXT
    if _EXPERT_CONTEXT is None:
        _EXPERT_CONTEXT = ExpertContext()
    return _EXPERT_CONTEXT


def set_expert_context(
    use_low_latency_ep: bool = False,
) -> ExpertContext:
    context = get_expert_context()
    context.use_low_latency_ep = use_low_latency_ep
    return context


def reset_expert_context() -> None:
    context = get_expert_context()
    context.reset_context()


def reset_expert_instance() -> None:
    global _EXPERT_CONTEXT
    _EXPERT_CONTEXT = None


def get_expert_runtime_context() -> ExpertContext:
    return get_expert_context()


def set_expert_runtime_context(
    use_low_latency_ep: bool = False,
) -> ExpertContext:
    return set_expert_context(use_low_latency_ep=use_low_latency_ep)


def reset_expert_runtime_context() -> None:
...
__all__ = [
    "ExpertContext",
    "get_expert_context",
    "get_expert_runtime_context",
    "reset_expert_instance",
    "reset_expert_context",
    "reset_expert_runtime_context",
    "set_expert_context",
    "set_expert_runtime_context",
]
