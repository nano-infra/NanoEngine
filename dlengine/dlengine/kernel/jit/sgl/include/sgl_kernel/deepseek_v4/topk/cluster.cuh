...
struct ClusterTopK {
    ...
    SGL_DEVICE static void stage1_init(void* _smem)
    {
        ...
        if (tx < kHistBins)
            smem->histogram[tx] = 0;
        if (tx < kNumStages)
            ptx::mbarrier_init(&smem->barrier[tx], 1);
        __syncthreads();
    }
...
#pragma unroll
        for (uint32_t stage = 0; stage < kNumStages; stage++) {
            ...
            if (lane_id == 0)
                ptx::mbarrier_wait(&smem->barrier[stage], 0);
            __syncwarp();
#pragma unroll
...
#pragma unroll
            for (uint32_t i = 0; i < kElemPerStage; ++i) {
                ...
                if (bin > thr_bin) {
                    ...
                    if (pos < kMaxTies)
                        smem->tie_buffer[pos] = {global_idx, score};
                }
            }
        }
        if (reuse) {
            ...
            if (tx < kHistBins)
                smem->histogram[tx] = 0;
            if (tx < num_stages)
                ptx::mbarrier_arrive(&smem->barrier[tx]);
        }
        __syncthreads();
    }
