...
class LoggerManager:
    ...
    @classmethod
    def get_logger(
        cls, name: str = "DLENGINE", use_relative_path: bool = True
    ) -> logging.Logger:
        """
        Retrieve the singleton logger instance. If it doesn't exist, create and configure it.

        Args:
            name (str, optional): The name/namespace of the logger. Defaults to "DLENGINE".
        ...
        if not logger.handlers:
            ...
            env_level = os.environ.get("DLENGINE_LOG_LEVEL", "").upper()
            ...
            # file_handler = logging.FileHandler('dlengine.log', encoding='utf-8')
...
def get_logger(
    name: str = "DLENGINE", use_relative_path: bool = True
) -> logging.Logger:
...
def set_log_level(level: str | int) -> None:
    """
    Set the log level for the DLENGINE logger.
    Args:
        level: "DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL" or logging level integer.
    """
    logger = LoggerManager.get_logger("DLENGINE")
    ...
    try:
        from dlengine._dlengine_rust import set_rust_log_level
