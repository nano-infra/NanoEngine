...
# Kimi K2 has a dense layer 0 followed by routed MoE layers 1-60. Record the
# first gate-up GEMM call for every MoE layer on every rank by default.
...
export NANODEPLOY_MOE_GEMM_DEBUG_LAYERS="${NANODEPLOY_MOE_GEMM_DEBUG_LAYERS:-all}"
export NANODEPLOY_MOE_GEMM_DEBUG_GEMMS="${NANODEPLOY_MOE_GEMM_DEBUG_GEMMS:-gate_up}"
export NANODEPLOY_MOE_GEMM_DEBUG_MAX_CALLS="${NANODEPLOY_MOE_GEMM_DEBUG_MAX_CALLS:-1}"
export NANODEPLOY_MOE_GEMM_DEBUG_SAMPLE_ELEMENTS="${NANODEPLOY_MOE_GEMM_DEBUG_SAMPLE_ELEMENTS:-8}"
...
{
  printf 'NANODEPLOY_MOE_GEMM_DEBUG=%q ' "$NANODEPLOY_MOE_GEMM_DEBUG"
  printf 'NANODEPLOY_MOE_GEMM_DEBUG_RANKS=%q ' "$NANODEPLOY_MOE_GEMM_DEBUG_RANKS"
  printf 'NANODEPLOY_MOE_GEMM_DEBUG_LAYERS=%q ' "$NANODEPLOY_MOE_GEMM_DEBUG_LAYERS"
  printf 'NANODEPLOY_MOE_GEMM_DEBUG_GEMMS=%q ' "$NANODEPLOY_MOE_GEMM_DEBUG_GEMMS"
  printf 'NANODEPLOY_MOE_GEMM_DEBUG_MAX_CALLS=%q ' "$NANODEPLOY_MOE_GEMM_DEBUG_MAX_CALLS"
  printf 'NANODEPLOY_MOE_GEMM_DEBUG_SAMPLE_ELEMENTS=%q ' "$NANODEPLOY_MOE_GEMM_DEBUG_SAMPLE_ELEMENTS"
...
grep -F '[NANODEPLOY_MOE_GEMM_INPUT]' "$LOG_PATH" || true
