...
SWEEP_SCRIPT="$SCRIPT_DIR/sweep_4node_variants.sh"

RAY_ADDR="${RAY_ADDR:-10.102.252.174:6379}"
MASTER_ADDR="${MASTER_ADDR:-10.102.252.174:29500}"

DEEPSEEK_MODEL="${DEEPSEEK_MODEL:-/mnt/nvme1n1/ml_research/chenjiefei/models/deepseek-v3}"
...
BENCH_VARIANTS="${BENCH_VARIANTS:-current,fixed_sp8,nccl,piecewise}"
ENABLE_PIECEWISE_VARIANT="${ENABLE_PIECEWISE_VARIANT:-1}"
ABLATION_RATES="${ABLATION_RATES:-}"
ABLATION_START_RATE="${ABLATION_START_RATE:-10}"
ABLATION_STEP_RATE="${ABLATION_STEP_RATE:-10}"
...
ABLATION_MAX_RATE="${ABLATION_MAX_RATE:-$MAX_RATE}"
...
RUN_TAG="${RUN_TAG:-issue001_4node_$(date -u +%Y%m%d_%H%M%S)}"
CHAIN_LOG_ROOT="${CHAIN_LOG_ROOT:-$ROOT_DIR/bench_logs/sp_ablation}"
CHAIN_LOG_DIR="${CHAIN_LOG_DIR:-$CHAIN_LOG_ROOT/$RUN_TAG}"
...
run_stage() {
    ...
    local stage_bench_variants="${16:-$BENCH_VARIANTS}"

    log "START phase=$phase model=$model_key dataset=$dataset_key strategy=$strategy sweep=$sweep_strategy routing=$routing_strategy dynamic_sp=$enable_dynamic_sp_size long_sp_size=$stage_long_request_sp_size variants=$stage_bench_variants start_rate=$start_rate step_rate=$step_rate max_rate=$max_rate batch_size=$stage_batch_size"
    ...
    env \
        ...
        GPU_MEM="$GPU_MEM" \
        GPU_UTIL="$GPU_UTIL" \
        PIECEWISE_GPU_UTIL="$PIECEWISE_GPU_UTIL" \
        MAX_MODEL_LEN="$MAX_MODEL_LEN" \
        MAX_INPUT_LEN="$MAX_INPUT_LEN" \
        LOOP_COUNT="$LOOP_COUNT" \
        RATE_DURATION_SEC="$RATE_DURATION_SEC" \
        FIXED_SP_SIZE="$FIXED_SP_SIZE" \
        SP_BACKEND="$SP_BACKEND" \
        CUDA_GRAPH_MODE="$CUDA_GRAPH_MODE" \
        BENCH_VARIANTS="$stage_bench_variants" \
        ENABLE_PIECEWISE_VARIANT="$ENABLE_PIECEWISE_VARIANT" \
        ...
        SLIME_VISIBLE_DEVICES="${SLIME_VISIBLE_DEVICES:-mlx5_bond_0}" \
        SLIME_GID_INDEX="${SLIME_GID_INDEX:-3}" \
        SLIME_QP_NUM="${SLIME_QP_NUM:-4}" \
        bash "$SWEEP_SCRIPT"

    record_stage "$phase" "$model_key" "$dataset_key" "$base_log_dir" "done"
    log "DONE phase=$phase model=$model_key dataset=$dataset_key strategy=$strategy sweep=$sweep_strategy routing=$routing_strategy dynamic_sp=$enable_dynamic_sp_size long_sp_size=$stage_long_request_sp_size variants=$stage_bench_variants start_rate=$start_rate step_rate=$step_rate max_rate=$max_rate batch_size=$stage_batch_size"
}
...
log "ABLATION_VARIANTS=$BENCH_VARIANTS"
if [[ -n "$ABLATION_RATES" ]]; then
    log "ABLATION_RATES=$ABLATION_RATES"
else
    log "ABLATION_SWEEP start=$ABLATION_START_RATE step=$ABLATION_STEP_RATE max=$ABLATION_MAX_RATE"
fi
...
# for long_sp_size in 2 4; do
    if [[ -n "$ABLATION_RATES" ]]; then
        ...
        start_rate_tag="${ABLATION_START_RATE//./p}"
        max_rate_tag="${ABLATION_MAX_RATE//./p}"
        step_rate_tag="${ABLATION_STEP_RATE//./p}"
        run_stage \
            "longshort" \
            "deepseek_v3" \
            "issue001" \
            ...
            "$ABLATION_START_RATE" \
            "$ABLATION_STEP_RATE" \
            "$CHAIN_LOG_DIR/longshort_issue001_deepseek_v3_sp${long_sp_size}_r${start_rate_tag}_${max_rate_tag}_step${step_rate_tag}" \
            "$BATCH_SIZE" \
            "$ABLATION_MAX_RATE" \
            "LeastBatch" \
            "$long_sp_size"
