...
RUN_TAG="${RUN_TAG:-issue001_issue003_issue005_mixed60k_longshort_dp4dp32_bs256_$(date -u +%Y%m%d_%H%M%S)}"
...
# for rate in 100; do
#     rate_tag="${rate//./p}"
#     run_stage "dp32" "deepseek_v3" "issue001" "$DEEPSEEK_MODEL" "$ISSUE001_DATASET" "legacy" "dp32sp1" "0" "$rate" "1" "$CHAIN_LOG_DIR/dp32_issue001_deepseek_v3_LB_r${rate_tag}" "$BATCH_SIZE" "$rate" "LeastBatch"
# done
