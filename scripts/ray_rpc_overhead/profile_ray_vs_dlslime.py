#!/usr/bin/env python3
"""Compare Ray Sequence transport with NanoDeploy's DLSLime data path.

This is a transport/control microbenchmark for the current checkout.  It uses
real Ray actors and the production ``RPCServerEndpoint``/
``RPCClientEndpoint`` classes, but deliberately does not start ModelRunner,
load a model, execute CUDA kernels, or run the scheduler.

The ``ray`` arm passes a full batch of NanoDeploy ``Sequence`` objects as the
Ray actor argument.  The ``dlslime`` arm sends the same objects through native
Sequence serialization and DLSLime/RDMA, while the actor receives an empty Ray
control command and calls ``recv_seqs``.  Both arms return the same synthetic
token rows through Ray so result fan-in is common to the comparison.

Actors reserve one GPU resource so PyTorch can allocate the pinned host buffer
required by the production DLSLime endpoint.  The benchmark does not load a
model, launch a ModelRunner, or execute any GPU kernel; the GPU is a resource
guard for the transport setup only.
"""
...
from nanodeploy._cpp import Sequence as NanoDeploySequence  # noqa: E402
from nanodeploy._cpp import serialize  # noqa: E402
from nanodeploy.endpoint import rpc_endpoint as rpc_endpoint_module  # noqa: E402
from nanodeploy.endpoint.rpc_endpoint import (  # noqa: E402
    RPCClientEndpoint,
    RPCServerEndpoint,
)
...
def _encode_transport_imm(payload_bytes: int, transport_slot: int) -> int:
    """Encode an immediate value for both old and slotted endpoints."""
    encode_imm = getattr(rpc_endpoint_module, "_encode_imm", None)
    if encode_imm is None:
        if transport_slot != 0:
            raise RuntimeError(
                "legacy RPC endpoint only supports transport slot 0"
            )
...
class RayVsDLSLimeWorker:
    """Minimal Ray-facing worker with an optional DLSLime endpoint."""
    ...
    def __init__(
        ...
        self.dlslime_buffer_bytes = dlslime_buffer_bytes
        self.endpoint: RPCClientEndpoint | None = None
    ...
    def init_dlslime_endpoint(self, server_info: list[object]) -> object:
        endpoint = RPCClientEndpoint(
            self.dlslime_buffer_bytes,
            self.logical_rank,
        )
        client_info = endpoint.init_client_endpoint()
    ...
    def _validate_sequences(self, sequences: Sequence[object]) -> None:
    ...
    def run(
        ...
        if is_prefill:
            raise ValueError("transport profiler measures the decode-shaped path")
        ...
        if enable_rpc:
            ...
            if self.endpoint is None:
                raise RuntimeError("DLSLime endpoint was not initialized")
    ...
    metadata = [
        {
            ...
            "is_head": bool(node_sort_key(node)[0] is False),
        }
...
def _profiled_send_seqs(
    ...
    *,
    ...
    """Run the production send loop with phase-level timing."""
    ...
    for rank in range(endpoint.world_size):
        ...
        buffer_ptr = buffer.data_ptr() + buffer.storage_offset() + slot_offset
    ...
    for future in futures:
        phase_start_ns = time.perf_counter_ns()
        future.wait()
        future_wait_ns_by_rank.append(time.perf_counter_ns() - phase_start_ns)
    ...
    return {
        ...
        "future_wait_ms_by_rank": [
            duration_ns / 1_000_000 for duration_ns in future_wait_ns_by_rank
        ],
...
def _invoke_once(
    ...
    *,
    ...
    if transport == "ray":
        ...
        refs = [actor.run.remote([], False, True, send_timestamp) for actor in actors]
    else:
    ...
    if transport == "dlslime":
        ...
        send_profile = _profiled_send_seqs(
            dlslime_endpoint,
            list(sequence_batches),
            is_prefill=False,
        )
        send_seqs_ms = (time.perf_counter_ns() - send_begin_ns) / 1_000_000

    ray_get_begin_ns = time.perf_counter_ns()
    results = ray.get(refs)
    end_ns = time.perf_counter_ns()
    receive_timestamp = time.time()
    submit_ms = (submit_end_ns - begin_ns) / 1_000_000
    ray_get_ms = (end_ns - ray_get_begin_ns) / 1_000_000
    roundtrip_ms = (end_ns - begin_ns) / 1_000_000
    ...
    *,
    ...
    try:
        for _ in range(measured_iterations):
            ...
            samples["dlslime_endpoint_total"].append(send_profile["total_ms"])
            samples["dlslime_serialize"].append(send_profile["serialize_ms"])
    ...
    record: dict[str, object] = {
        ...
        "logical_input_token_ids": logical_workers
        * batch_size_per_gpu
        * sequence_length,
        ...
        "ray_output_pickle_bytes_total": logical_workers * output_bytes_per_worker,
    }
    ...
    record["dlslime_future_wait_mean_ms_by_rank"] = [
        statistics.fmean(values) for values in future_wait_samples_by_rank
    ]
    record["dlslime_future_wait_p99_ms_by_rank"] = [
        _percentile(values, 99.0) for values in future_wait_samples_by_rank
    ]
...
def _write_results(
    ...
    if not records:
        raise ValueError("at least one transport record is required")
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.logical_workers % args.workers_per_node != 0:
        parser.error("--logical-workers must be divisible by --workers-per-node")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ray_context = ray.init(
        ...
        logging_level="ERROR",
        log_to_driver=False,
        # The external cluster may have been started before the SLIME
        # variables were exported.  Propagate the transport settings with the
        # job as well, so every actor uses the same NIC/GID/QP configuration.
        runtime_env={
            "env_vars": {
                name: os.environ[name]
                for name in REQUIRED_SLIME_ENV
    ...
    try:
        ...
        for transport in args.transports:
            ...
            for batch_size_per_gpu in args.batch_sizes:
                ...
                dlslime_input_bytes_total = (
                    _native_dlslime_bytes(server_endpoint, sequence_batches)
                ...
                if transport == "dlslime":
                    print(
                        ...
                        "future.wait="
                        f"{record['dlslime_future_wait_mean_ms']:.3f} ms | "
                        "unattributed="
                        f"{record['dlslime_unattributed_mean_ms']:.3f} ms",
                        flush=True,
                    )
    finally:
        for actor in actors:
    ...
    metadata = {
        ...
        "slime_environment": {
            name: os.environ.get(name) for name in REQUIRED_SLIME_ENV
        },
        "timed_scope": (
            "Ray actor submission; full Sequence input via Ray or native "
            "NanoDeploy serialize + DLSLime send_seqs/recv_seqs; synthetic "
            "token output via Ray; ray.get fan-in; DLSLime phase breakdown"
        ),
        "excluded_scope": (
            "ModelRunner, model weights, CUDA kernels, scheduler, request "
            "ingress, worker collectives, and end-to-end TTFT/TPOT"
        ),
        "arguments": {
